...
def wait_for_cheap_ci(
    *,
    ...
    print(f"Starting wait for cheap CI on repo={repository} commit={commit_sha} timeout={timeout_seconds}s")
    
    while True:
        elapsed = int(time.monotonic() - start_time)
        if elapsed > timeout_seconds:
            print(f"::error::Timeout waiting for CI workflow run to be registered/finish for commit {commit_sha}")
            return 1
        ...
        if not runs:
            print(f"CI workflow run not found yet. Elapsed {elapsed}s. Retrying in {poll_seconds}s...")
        ...
        if successful_runs:
            selected_run = sorted(successful_runs, key=lambda r: int(r["id"]), reverse=True)[0]
        elif active_runs:
            selected_run = sorted(active_runs, key=lambda r: int(r["id"]), reverse=True)[0]
        else:
            selected_run = sorted(runs, key=lambda r: int(r["id"]), reverse=True)[0]
            
        run_id = int(selected_run["id"])
        print(f"Selected CI workflow run ID: {run_id} (status: {selected_run.get('status')}, conclusion: {selected_run.get('conclusion')})")
        ...
        if jobs:
            ...
            if all_done:
                if all_success:
                    print("✅ Both cheap CI jobs (Lint and AC Traceability Check) succeeded!")
                    return 0
                else:
                    ...
                    if not active_runs and len(successful_runs) <= 0:
                        ...
                        print("Selected run failed/completed, but other active or successful runs exist. Retrying...")
                        
        print(f"Waiting for cheap CI jobs. Elapsed {elapsed}s. Retrying in {poll_seconds}s...")
        time.sleep(poll_seconds)
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--poll-seconds", type=int, default=15, help="Poll interval in seconds")
    parser.add_argument("--timeout-seconds", type=int, default=600, help="Max wait timeout in seconds")
