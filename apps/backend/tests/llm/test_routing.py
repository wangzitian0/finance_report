"""Provider routing maps families onto litellm calls (EPIC-023 AC-llm.2.1)."""
...
def test_AC23_2_1_openai_compatible_prefixes_and_keeps_api_base():
    """AC-llm.2.1: openai-compatible (Z.AI/GLM, vLLM…) -> openai/ prefix + custom api_base."""
...
def test_AC23_2_1_openrouter_keeps_vendor_segment_and_adds_headers():
    """AC-llm.2.1: OpenRouter vendor/model is preserved (not stripped) + attribution headers."""
...
def test_AC23_2_1_anthropic_is_native_without_api_base():
    """AC-llm.2.1: anthropic-compatible -> anthropic/ prefix, no custom api_base."""
...
def test_AC23_2_1_gemini_is_native_and_drops_inherited_api_base():
    """AC-llm.2.1: google-gemini -> gemini/ prefix on litellm's native endpoint; any
...
def test_AC23_2_1_avoids_double_family_prefix():
    """AC-llm.2.1: a model already carrying the litellm family token is not double-prefixed."""
...
def test_AC23_2_1_kwargs_only_include_set_fields():
    """AC-llm.2.1: kwargs() omits api_base/headers when not applicable."""
