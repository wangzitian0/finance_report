...
async def test_buy_transaction_creates_balanced_journal_entry_and_lot(
    ...
    """AC-portfolio.accounting.1: Buy transactions increase investments and reduce brokerage cash."""
...
async def test_sell_transaction_uses_fifo_and_records_realized_gain(
    ...
    """AC-portfolio.lot.1: Sell transactions consume FIFO lots and record realized P&L."""
    await svc.post_buy(
        db,
        user_id=test_user.id,
        transaction_date=date(2026, 1, 5),
...
async def test_sell_transaction_uses_average_cost_for_realized_pnl(
    ...
    """AC-portfolio.lot.2: Average cost basis is explicit and persisted on sell transactions."""
    await svc.post_buy(
        db,
        user_id=test_user.id,
        transaction_date=date(2026, 1, 5),
...
async def test_sell_transaction_uses_lifo_loss_and_disposes_position(
    ...
    """AC-portfolio.position.1: LIFO sells can realize losses and close positions."""
    await svc.post_buy(
        db,
        user_id=test_user.id,
        transaction_date=date(2026, 1, 5),
...
async def test_dividend_transaction_posts_income_and_dividend_record(
    ...
    """AC-portfolio.dividend.1: Dividends create cash, income, and DividendIncome records."""
...
async def test_dividend_transaction_posts_withholding_tax(
    ...
    """AC-portfolio.dividend.2: Withholding tax splits dividend cash and tax expense."""
    await svc.post_buy(
        db,
        user_id=test_user.id,
        transaction_date=date(2026, 1, 5),
...
async def test_investment_accounting_rejects_invalid_transactions(
    ...
    """AC-portfolio.accounting.2: Invalid transaction inputs are rejected."""
...
async def test_investment_accounting_rejects_invalid_account_and_position_helpers(
    ...
    """AC-portfolio.accounting.3: Ledger helpers reject invalid accounts and missing positions."""
    user_id = uuid4()
    account_id = uuid4()
