...
class TestRepairJsonObject:
    def test_strips_json_code_fence(self):
        """AC-extraction.114.1: A ```json fenced object is recovered."""
    ...
    def test_strips_bare_code_fence_and_prose(self):
        """AC-extraction.114.2: Surrounding prose and a bare ``` fence are stripped to the
    ...
    def test_strips_single_line_fence(self):
        """AC-extraction.114.1b: A single-line fenced block (no newline) is recovered."""
    ...
    def test_clean_object_is_preserved(self):
        """AC-extraction.114.3: An already-clean object round-trips unchanged."""
        content = '{"x": "y"}'
        repaired = ExtractionService._repair_json_object(content)
        assert repaired is not None
    ...
    def test_unrecoverable_returns_none(self):
        """AC-extraction.114.4: Content with no JSON object is unrecoverable."""
    ...
    def test_does_not_misread_braces_in_strings(self):
        """AC-extraction.114.4b: Braces inside string values do not truncate the object."""
    ...
    def test_unbalanced_open_brace_returns_none(self):
        """AC-extraction.114.4c: An opening brace with no matching close (truncated response)
    ...
    def test_escaped_quote_in_string_does_not_end_object_early(self):
        """AC-extraction.114.4d: A backslash-escaped quote inside a string value does not end
    ...
    def test_prefers_largest_object_when_example_precedes_real(self):
        """AC-extraction.114.7: when a small example object precedes the real (larger)
    ...
    def test_complete_object_then_trailing_unbalanced_brace(self):
        """AC-extraction.114.8: a complete object followed by trailing junk that opens an
    ...
    def test_leading_unbalanced_brace_then_real_object(self):
        """AC-extraction.114.9: a leading unmatched brace (junk) before the real object does
...
class TestExtractionSalvagesFencedResponse:
    async def test_fenced_response_is_salvaged(self):
        """AC-extraction.114.5: A fenced (otherwise-valid) model response is salvaged by the
        ...
        with (
            ...
            patch("src.extraction.extension.service.stream_ai_json", return_value=MagicMock()),
            patch("src.extraction.extension.service.accumulate_stream", AsyncMock(return_value=fenced)),
        ):
    ...
    async def test_unrecoverable_response_still_fails(self):
        """AC-extraction.114.6: A response with no recoverable JSON still fails as before."""
        service = ExtractionService()
        ...
        with (
            ...
            patch.object(service, "base_url", "https://test.api"),
            patch("src.extraction.extension.service.stream_ai_json", return_value=MagicMock()),
            patch("src.extraction.extension.service.accumulate_stream", AsyncMock(return_value="not json at all")),
        ):
