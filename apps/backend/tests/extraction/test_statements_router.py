...
async def test_upload_statement_duplicate(db, monkeypatch, storage_stub, test_user):
    """Uploading same file twice should trigger duplicate detection."""
    ...
    mock_get_model_info = AsyncMock(return_value={"id": "test-model", "input_modalities": ["image"]})
    monkeypatch.setattr(statements_router, "get_model_info", mock_get_model_info)

    mock_model_matches = MagicMock(return_value=True)
    monkeypatch.setattr(statements_router, "model_matches_modality", mock_model_matches)
