...
async def test_delete_account_with_transactions_fails(client: AsyncClient, db: AsyncSession, test_user):
    ...
    entry = JournalEntry(user_id=test_user.id, entry_date=date.today(), memo="Txn", source_type="manual", status=JournalEntryStatus.DRAFT)
    db.add(entry)
    await db.flush()
    
    line = JournalLine(journal_entry_id=entry.id, account_id=acc.id, direction=Direction.DEBIT, amount=Decimal("10"), currency="SGD")
    db.add(line)
    await db.commit()
    await db.refresh(acc)
...
async def test_delete_draft_journal_entry(client: AsyncClient, db: AsyncSession, test_user):
    # 1. Create draft
    entry = JournalEntry(user_id=test_user.id, entry_date=date.today(), memo="Draft", source_type="manual", status=JournalEntryStatus.DRAFT)
    db.add(entry)
    await db.commit()
...
async def test_delete_posted_journal_entry_fails(client: AsyncClient, db: AsyncSession, test_user):
    # 1. Create posted
    entry = JournalEntry(user_id=test_user.id, entry_date=date.today(), memo="Posted", source_type="manual", status=JournalEntryStatus.POSTED)
    db.add(entry)
    await db.commit()
    await db.refresh(entry)
...
async def test_delete_statement(client: AsyncClient, db: AsyncSession, test_user):
    ...
    # Since we are mocking storage or relying on S3 service, it might fail if S3 not configured or reachable.
    # However, our code catches Exception log warning and proceeds to DB delete.
    # So unit test should pass 204 regardless of S3 state (unless storage instantiation fails hard).
    # StorageService init might fail if env vars missing. But test_storage.py passes so env likely has defaults.
