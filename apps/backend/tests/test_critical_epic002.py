...
@pytest.mark.asyncio
async def test_many_lines_complex_salary_breakdown(
    db: AsyncSession, test_user_id
):
    """
    HIGH #9: Multi-line complex entry (salary breakdown with deductions).
    
    Salary entry with 6+ lines:
    - Bank (Asset) DEBIT: Net salary
    - CPF Employee (Liability) CREDIT: Employee contribution
    - Income Tax (Liability) CREDIT: Tax withheld
    - Health Insurance (Expense) DEBIT: Employee share
    - Salary Income (Income) CREDIT: Gross salary
    """
    # Create accounts
    accounts = []
    account_types = [
        ("Bank", AccountType.ASSET),
        ("CPF Employee", AccountType.LIABILITY),
        ("Income Tax Payable", AccountType.LIABILITY),
        ("Health Insurance Expense", AccountType.EXPENSE),
    ...
    # Gross salary: 5000 + 500 bonus = 5500
    # Deductions: CPF 1000, Tax 500, Health 200 = 1700
    # Net: 3800
    lines_data = [
        (bank.id, Direction.DEBIT, "3800.00"),      # Net to bank
        (cpf.id, Direction.CREDIT, "1000.00"),      # CPF deduction
        (tax.id, Direction.CREDIT, "500.00"),       # Tax withheld
        (health.id, Direction.DEBIT, "200.00"),     # Health insurance expense
    ...
    # Post the entry - should succeed with 6 lines
    ...
    total_credit = sum(
        line.amount for line in entry.lines if line.direction == Direction.CREDIT
    )
    assert total_debit == Decimal("4000.00")  # 3800 + 200
    assert total_credit == Decimal("6000.00")  # 1000 + 500 + 4500 + 500

    # Wait, the entry should be balanced. Let me recalculate...
    # DEBIT: Bank 3800 + Health 200 = 4000
    # CREDIT: CPF 1000 + Tax 500 + Salary 4500 + Bonus 500 = 6500
    # This is NOT balanced! Let me fix...
