...
async def test_statement_router_error_cases(db, test_user, monkeypatch):
    ...
    statement = BankStatement(
        id=sid, user_id=uid, status=BankStatementStatus.PARSING,
        file_path="p", file_hash="h", original_filename="f.pdf", institution="DBS"
    )
    db.add(statement)
    await db.commit()
    with pytest.raises(HTTPException) as exc:
    ...
    await _parse_statement_background(
        statement_id=uuid4(), filename="f", institution="i", user_id=uid,
        account_id=None, file_hash="h", storage_key="k", content=b"", 
        model=None, session_maker=session_maker
    )
...
async def test_parse_statement_background_error_paths(db, test_user):
    ...
    statement = BankStatement(
        id=sid, user_id=uid, status=BankStatementStatus.PARSING,
        file_path="p", file_hash="h2", original_filename="f.pdf", institution="DBS"
    )
    db.add(statement)
    await db.commit()
    ...
    # 1. StorageError in background
    with patch("src.routers.statements.StorageService") as mock_storage_cls:
        mock_storage = mock_storage_cls.return_value
        mock_storage.generate_presigned_url.side_effect = StorageError("S3 Fail")
        
        await _parse_statement_background(
            statement_id=sid, filename="f", institution="i", user_id=uid,
            account_id=None, file_hash="h2", storage_key="k", content=b"", 
            model=None, session_maker=session_maker
        )
        
        await db.refresh(statement)
        assert statement.status == BankStatementStatus.REJECTED
        assert "S3 Fail" in statement.validation_error
...
async def test_retry_statement_parsing_error_paths(db, test_user):
    ...
    statement = BankStatement(
        id=sid, user_id=uid, status=BankStatementStatus.PARSED,
        file_path="p", file_hash="h3", original_filename="f.pdf", institution="DBS"
    )
    db.add(statement)
    await db.commit()
