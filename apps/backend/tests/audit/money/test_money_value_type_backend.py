"""Money value-type behavioural proofs (backend module (src.audit.money) — EPIC-002 AC2.19–AC2.21, #1167 / #1171).

These exercise the backend ``src.audit.money`` narrow waist: ``Money`` / ``Currency``
...
def test_AC2_20_1_convert_rounds_half_even_at_boundary():
    ...
    assert convert(
        Money(Decimal("1.00"), "USD"), ExchangeRate("USD", "EUR", Decimal("1.005")), rounding="ROUND_HALF_UP"
...
def test_AC2_21_1_balance_rejects_currency_mismatch_and_duplicates():
    ...
    from src.audit.money.errors import MoneyError
