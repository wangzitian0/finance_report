...
The cross-language standard lives in ``common/audit/money/conformance/vectors.json`` and
the Python reference impl (``common/audit/money``) is proven against it in the tooling
lane. This test proves the **actually-shipped backend rounding path**
(``src.audit.money.to_money`` — imported across routers/services) also conforms to
