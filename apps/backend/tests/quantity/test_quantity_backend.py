...
_VECTORS = json.loads((Path(__file__).resolve().parents[4] / "common/quantity/conformance/vectors.json").read_text())
...
def test_AC12_30_2_backend_quantity_quantize_matches(case):
    """AC12.30.2: src.quantity quantize matches the shared standard."""
...
def test_AC12_30_1_backend_quantity_value_type_laws():
    """AC12.30.1: src.quantity rejects float, uses 6-dp policy, and guards units."""
