...
async def _bound_scene_model(user_id: UUID | None) -> str | None:
    """The bare model a user bound to ``advisor.chat`` (EPIC-023 AC23.4.5), or None.

    Best-effort: ``user_id is None`` or any config-resolution error falls back to
    the env model list rather than breaking chat. The binding's ``model_id`` is
    qualified ``provider_id/model``; the bare model is what the transport sends
    (the provider is resolved separately, per user, in ``ai_streaming``)."""
    if user_id is None:
        return None
    try:
    ...
    if binding is None:
        return None
    return binding.model_id.split("/", 1)[-1]
...
class AIAdvisorService:
    ...
    async def chat_stream(
        ...
        # per-message model, else the user's advisor.chat bound model, else the env
        # primary. Otherwise a bound model would share the env-primary's cache entry
        # and serve a response generated by a different model.
        model_key = model or await _bound_scene_model(user_id) or self.primary_model
    ...
    async def _stream_and_store(
        ...
        try:
            async for chunk, model_name in self._stream_openrouter(messages, preferred_model, user_id):
    ...
    async def _stream_openrouter(
        self, messages: list[dict[str, str]], preferred_model: str | None, user_id: UUID | None = None
        ...
        if preferred_model:
            ...
            # model (EPIC-023 AC23.4.5) so /settings/llm actually takes effect.
            bound = await _bound_scene_model(user_id)
            if bound:
                models = [bound, *models]
