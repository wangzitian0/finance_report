...
``reconciliation``/``llm``/``audit``); reads whose owner still lives in the
app remainder are injected through the ``extension/app_reads.py`` ports,
wired by the composition root (``src/main.py``).
...
from src.advisor.extension.app_reads import (
