...
class OpeningPositionPolicy:
    ...
    def fold(self, parents: Sequence[TraceRecord]) -> TraceDecisionOutcome:
        inputs = [
            ...
            if parent.assertion == VersionedTraceRef("ledger_opening_input", "starting-stock", "1")
        ]
        sources = [parent for parent in parents if parent not in inputs]
        valid = (
            len(inputs) == 1
            and inputs[0].result is TraceResult.PASS
            and len(sources) <= 1
            and all(
                parent.result is TraceResult.AUTHORITATIVE and parent.target_class is TraceTargetClass.FINANCIAL
...
async def record_opening_position(
    ...
    *,
    ...
    emitter = trace_emitter or TraceEmitter(SqlTraceRecordRepository(db, TraceDecisionPolicyRegistry((policy,))))
    repository = emitter.repository
    parents = [observation]
    if source_decision_id is not None:
        if trace_emitter is None:
            raise ValueError("Source-backed opening requires a composed trace emitter")
        source = await repository.get(scope, source_decision_id)
        if source is None or source.result is not TraceResult.AUTHORITATIVE:
            raise ValueError("Opening source decision is not authoritative")
    ...
    await emitter.emit_many((observation, decision))
    versions = (
...
async def list_opening_positions(db: AsyncSession, *, user_id: UUID, as_of: date) -> tuple[OpeningPosition, ...]:
    ...
    for row in records:
        ...
        source_evidence = authorities.get(row.source_decision_id) if valid and row.source_decision_id else None
        source_decision = (
            TraceDecisionRef(
                row.source_decision_id,
                VersionedTraceRef(
                    source_evidence.target_kind, source_evidence.target_id, source_evidence.target_version
                ),
                VersionedTraceRef(
                    source_evidence.assertion_kind, source_evidence.assertion_id, source_evidence.assertion_version
                ),
            )
            if source_evidence is not None and row.source_decision_id is not None
            else None
        )
        positions.append(
            OpeningPosition(
    ...
    for historical_entry in entries:
        if historical_entry.id in entry_ids or historical_entry.decision_anchor_id is None:
            continue
        evidence = authorities.get(historical_entry.decision_anchor_id)
        if (
            evidence is None
            ...
            continue
        for line in historical_entry.lines:
            acct = line.account
            ...
            decision = TraceDecisionRef(
                historical_entry.decision_anchor_id,
