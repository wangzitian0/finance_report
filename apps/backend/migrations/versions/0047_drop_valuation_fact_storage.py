"""drop atomic valuation facts + classification storage (retire reinvention island)

The valuation taxonomy stack (#1221-1224) reinvented the existing accounting
primitives (chart of accounts, double-entry ledger, transaction classification,
evidence graph). It was an orphaned island: nothing ever wrote these tables.
Manual/non-bank valuations will instead flow through the existing
Account/JournalEntry/TransactionClassification pipeline. These two tables are
empty, so dropping them is data-safe.
"""

import sqlalchemy as sa
from alembic import op

revision = "0047_drop_valuation_fact_storage"
down_revision = "0046_valuation_fact_storage"
branch_labels = None
depends_on = None
...
def downgrade() -> None:
    # Recreate the tables as defined in revision 0046 (no data is restored; the
    # island was never populated).
    _l1 = (
        "cash",
