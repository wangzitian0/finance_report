...
const VECTORS = JSON.parse(readFileSync(vectorsPath, "utf-8")) as {
  ...
  ratio: {
    ...
    whole: string;
    unit: string;
    expected_percent_2dp: string;
  }[];
};

describe("quantity conformance (cross-language standard AC12.30)", () => {
  it("AC-audit.30.2 declares the same quantum and default rounding", () => {
    expect(QUANTITY_DP).toBe(VECTORS.quantity_dp);
    expect(QUANTITY_QUANTUM.equals(new Decimal(VECTORS.quantity_quantum))).toBe(
      true,
    );
  ...
  it("AC-audit.30.2 matches every quantize vector", () => {
  ...
  it("AC-audit.30.2 normalizes and rejects units per the standard", () => {
  ...
  it("AC-audit.30.1 AC-audit.30.2 derives ratios from same-unit quantities", () => {
    for (const c of VECTORS.ratio) {
      const got = new Quantity(c.part, c.unit).ratioTo(
        new Quantity(c.whole, c.unit),
      );
      expect(
        got.toPercent().equals(new Decimal(c.expected_percent_2dp)),
        JSON.stringify(c),
      ).toBe(true);
    }
  });
...
describe("quantity value-type laws (TS rendering of the contract)", () => {
  it("AC-audit.30.1 rejects JS number values", () => {
    ...
    expect(() => new Quantity("Infinity", "shares")).toThrow(
      FloatNotAllowedError,
    );
  });

  it("AC-audit.30.1 guards same-unit arithmetic and comparison", () => {
    ...
    expect(
      decimalInput
        .quantize(Decimal.ROUND_HALF_DOWN)
        .value.equals(new Decimal("1.234567")),
    ).toBe(true);
    ...
    expect(() => a.add(new Quantity("1", "contracts"))).toThrow(
      UnitMismatchError,
    );
  });

  it("AC-audit.30.4 formats quantities from the quantity package", () => {
