...
//
// Proves the frontend rendering of the money standard: AC-audit.19.1 (Currency
// validation), AC-audit.20.1 (rounding + convert), AC2.21.1 (the value-type laws).
...
describe("money conformance (cross-language standard #1167)", () => {
  it("AC-audit.20.1 declares the same quantum and default rounding", () => {
    expect(VECTORS.money_quantum).toBe("0.01");
    expect(VECTORS.default_rounding).toBe("ROUND_HALF_EVEN");
  });

  it("AC-audit.20.1 matches every rounding vector (banker's rounding, not HALF_UP)", () => {
  ...
  it("AC-audit.30.3 matches every convert vector through ExchangeRate", () => {
    for (const c of VECTORS.convert) {
      const result = convert(
        new Money(c.amount, c.from),
        new ExchangeRate(c.from, c.to, c.rate),
        c.rounding,
      );
      expect(
        result.amount.equals(new Decimal(c.expected)),
        JSON.stringify(c),
      ).toBe(true);
  ...
  it("AC-audit.19.1 normalizes and rejects currencies per the standard", () => {
  ...
  it("AC-audit.19.1 embeds exactly the canonical ISO-4217 set", () => {
    expect([...ISO_4217_CODES].sort()).toEqual([...VECTORS.iso4217].sort());
  });
});

describe("money value-type laws (TS rendering of the contract)", () => {
  it("AC-audit.19.1 AC-audit.30.3 rejects float (JS number) amounts and rates", () => {
    ...
    expect(() => new ExchangeRate("USD", "EUR", 1.2)).toThrow(
      FloatNotAllowedError,
    );
  });

  it("AC-audit.19.2 is same-currency only for arithmetic and comparison", () => {
    const usd = new Money("10.00", "USD");
    const sgd = new Money("10.00", "SGD");
    expect(Money.zero("USD").equals(new Money("0", "USD"))).toBe(true);
    expect(
      usd.add(new Money("5", "USD")).amount.equals(new Decimal("15")),
    ).toBe(true);
    expect(
      usd.subtract(new Money("2", "USD")).amount.equals(new Decimal("8")),
    ).toBe(true);
  ...
  it("AC-audit.19.1 AC-audit.30.3 rejects non-finite Decimal amounts and rates", () => {
    expect(() => new Money(new Decimal(Infinity), "USD")).toThrow(
      FloatNotAllowedError,
    );
    expect(() => new ExchangeRate("USD", "EUR", new Decimal(Infinity))).toThrow(
      FloatNotAllowedError,
    );
  });

  it("AC-audit.30.3 uses ExchangeRate as the typed conversion boundary", () => {
    ...
    expect(
      convert(new Money("100.00", "USD"), rate).equals(
        new Money("135.00", "SGD"),
      ),
    ).toBe(true);
    expect(
      convert(new Money("135.00", "SGD"), rate.inverse()).equals(
        new Money("100.00", "USD"),
      ),
    ).toBe(true);
    expect(() => convert(new Money("100.00", "EUR"), rate)).toThrow(
      CurrencyMismatchError,
    );
    expect(() => new ExchangeRate("USD", "SGD", "0")).toThrow();
    // @ts-expect-error convert rejects non-ExchangeRate objects at runtime too
    expect(() => convert(new Money("100.00", "USD"), "1.35")).toThrow(
      FloatNotAllowedError,
    );
  });
});
