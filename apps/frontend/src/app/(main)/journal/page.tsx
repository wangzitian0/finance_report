...
export default function JournalPage() {
  ...
  const calculateDebits = (lines: JournalLine[]) => {
    return sumAmounts(
      lines.filter((l) => l.direction === "DEBIT").map((line) => line.amount),
    );
  };
  ...
  const voidDialog = useConfirmDialog<[string]>(async (entryId, reason) => {
    try {
      ...
      showToast(
        "Entry voided successfully. Reversal entry created.",
        "success",
      );
      fetchEntries();
    } catch (err) {
