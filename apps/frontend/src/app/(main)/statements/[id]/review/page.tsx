...
import {
  SourceEnvelopeConfirmation,
  type CustodyAccount,
  type EnvelopeDraft,
} from "@/components/review/SourceEnvelopeConfirmation";
import {
...
export default function StatementReviewPage() {
  ...
  return (
    <div className="flex min-h-[calc(100dvh-2rem)] w-full max-w-full min-w-0 flex-col overflow-x-hidden p-4 md:p-6 2xl:h-[calc(100dvh-2rem)]">
      ...
      <SourceEnvelopeConfirmation
        requiresEnvelopeConfirmation={requiresEnvelopeConfirmation}
        requiresOtherSourceReview={requiresOtherSourceReview}
        sourceMissingFacts={sourceMissingFacts}
        missingFactLabels={missingFactLabels}
        envelopeDraft={envelopeDraft}
        onDraftChange={setEnvelopeDraft}
        custodyAccounts={custodyAccountsData?.items || []}
        onConfirm={() => envelopeMutation.mutate()}
        isPending={envelopeMutation.isPending}
        reviewedEnvelope={data.reviewed_envelope}
      />
