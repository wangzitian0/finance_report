...
function getRangeParams(range: NetWorthRange): {
  from: string;
  ...
  return {
    from: formatDateInput(addMonths(today, months)),
    granularity: "daily",
  };
}
  ...
  const fetchSeries = useCallback(async () => {
    ...
    try {
      const response = await apiOperation(
        ...
        {
          query: { from: params.from, to, granularity: params.granularity },
        },
      );
      setData(response);
      setError(null);
    ...
    return {
      ...
      tooltip: {
        ...
        valueFormatter: (value: number) =>
          formatCurrencyLocale(value, data?.currency ?? "SGD"),
      },
  ...
  return (
    <section className="card p-5" aria-label="Net worth time-series">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
        ...
        <div
          className="flex gap-1 bg-[var(--background-muted)] p-1 rounded-lg w-fit"
          role="tablist"
      ...
      {loading ? (
        ...
        <p className="text-sm text-muted">
          At least two net worth points are needed to draw a line.
        </p>
      ) : (
