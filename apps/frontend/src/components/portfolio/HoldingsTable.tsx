...
export function HoldingsTable({ holdings, showDisposed = false }: HoldingsTableProps) {
    const filtered = showDisposed ? holdings : holdings.filter((h) => h.status === "active");
    ...
    if (filtered.length === 0) {
        return (
            <div className="card p-8 text-center">
                ...
                <p className="text-sm text-muted mt-1">Upload brokerage statements and reconcile to see holdings.</p>
            </div>
        );
    }

    // Group by broker/account
    const grouped = filtered.reduce((groups, h) => {
                ...
                <div key={broker} className="card">
                    <div className="card-header flex items-center justify-between">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs text-muted">{brokerHoldings.length} holdings</span>
                        </div>
                    </div>
                            ...
                            <tbody className="divide-y divide-[var(--border)]">
                                {brokerHoldings.map((h) => (
                                    <tr key={h.id} className="hover:bg-[var(--background-muted)]/50 transition-colors">
                                        ...
                                        <td className={`px-4 py-2 text-right ${getPnlColor(h.unrealized_pnl_percent)}`}>
                                            {formatPnlPercent(h.unrealized_pnl_percent)}
                                        </td>
                                        <td className="px-4 py-2">
                                            <span className={`badge ${h.status === "active" ? "badge-success" : "badge-muted"}`}>
                                                {h.status}
                                            </span>
                                        </td>
                                    </tr>
