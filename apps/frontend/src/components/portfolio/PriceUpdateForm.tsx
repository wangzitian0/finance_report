...
const EMPTY_ROW: PriceRow = { asset_identifier: "", price: "", currency: "USD", price_date: todayStr() };
...
export function PriceUpdateForm({ knownTickers = [], onSuccess }: PriceUpdateFormProps) {
    const { showToast } = useToast();
    const [rows, setRows] = useState<PriceRow[]>([{ ...EMPTY_ROW }]);

    const mutation = useMutation({
        mutationFn: (updates: PriceUpdate[]) =>
            apiFetch<PriceUpdateResponse>("/api/portfolio/prices/update", {
                method: "POST",
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const valid = rows.filter((r) => r.asset_identifier.trim() && r.price.trim());
        if (valid.length === 0) {
                        ...
                        <div className="flex-1">
                            ...
                            {knownTickers.length > 0 ? (
                                <select
                                    ...
                                    onChange={(e) => updateRow(index, "asset_identifier", e.target.value)}
                                    className="input"
                                    required
                                >
