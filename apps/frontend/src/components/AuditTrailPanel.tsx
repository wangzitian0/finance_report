...
export default function AuditTrailPanel({ transactionId }: AuditTrailPanelProps) {
  ...
  const fetchAuditTrail = useCallback(async () => {
    setLoading(true);
    try {
      const data = await apiFetch<{ items: AuditTrailRecord[] }>(`/api/transactions/${transactionId}/audit`);
    ...
    <section className="border border-[var(--border)] rounded-lg overflow-hidden">
      ...
      {loading ? (
        ...
        <div className="divide-y divide-[var(--border)]">
          {records.map((record) => (
            <div key={`${record.timestamp}-${record.actor}-${record.action}`} className="p-4 text-sm">
              <div className="flex items-center justify-between gap-3 mb-2">
                <div className="flex items-center gap-2">
                  <span className="badge badge-muted">{record.actor}</span>
                  <span className="font-medium">{record.action}</span>
                </div>
                <span className="text-xs text-muted">{formatDateTimeDisplay(record.timestamp)}</span>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-xs">
                <div className="rounded bg-[var(--background-muted)]/40 p-2">
                  <span className="text-muted">Old:</span> {formatValue(record.old_value)}
                </div>
                <div className="rounded bg-[var(--background-muted)]/40 p-2">
                  <span className="text-muted">New:</span> {formatValue(record.new_value)}
                </div>
              </div>
            </div>
          ))}
