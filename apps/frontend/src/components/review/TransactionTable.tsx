    ...
    <div className="card flex h-full min-h-0 min-w-0 max-w-full flex-col overflow-hidden">
      <div className="card-header flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <span className="text-xs text-muted">
          {transactionRows.length} total
        </span>
      </div>

      <div
        data-testid="stage1-mobile-transaction-list"
        className="min-w-0 flex-1 divide-y divide-[var(--border)] overflow-y-auto md:hidden"
      >
      ...
      <div
        data-testid="stage1-desktop-transaction-region"
        ...
        >
          ...
          <tbody className="divide-y divide-[var(--border)]">
            {transactionRows.map((txn) => (
              <tr
                key={txn.id}
                className="hover:bg-[var(--background-muted)]/50 group"
              >
