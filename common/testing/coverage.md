...
The authoritative component/file policy lives in `common/meta/extension/coverage/policy.py`. Coverage tools must emit LCOV source paths that match that policy:
...
Each component carries an explicit `tier` on `common/meta/extension/coverage/policy.py::CoverageComponent`:
...
pattern in `common/meta/extension/coverage/policy.py::COVERAGE_EXEMPT_PATTERNS` (tests, config,
...
- **Output**: `coverage-backend-{shard}.lcov` (5 shards, merged into `coverage/backend.lcov`)
- **Excluded**:
  - `tests/**`
  - `migrations/**`
  - `__init__.py` files
...
jobs:
  backend:
    # 5 shards -> coverage-backend-{1..5}.lcov
...
The new source file must either appear in the matching LCOV report or be explicitly excluded in `common/meta/extension/coverage/policy.py`. Prefer adding tests/import coverage for real modules. Only exclude generated, type-only, test-only, or entrypoint files.
...
explicitly excluded by `common/meta/extension/coverage/policy.py`. CI must not silently shrink
