...
changes (Classify Changes) ──→ backend shards ───────┬→ ac-behavioral-ratchet ───────┤
                         ...
                         ├────→ tooling-coverage (2-way) ─→ tooling-coverage-merge ─┬→ unified-coverage ─┤
                         │             └──────────────┐                               │
                         ├────────────────────────────┴→ ac-traceability ─────────────┤
                         ├────→ schema-migrations ───────────────────────────────────┤
                         ├────→ frontend-build ──────────────────────────────────────┤
...
| **backend** (Shards 1-4) | Backend fast-path tests only: `-m "not slow and not e2e and not integration"`, each shard running only the test files its seeded file-level split assigns (`tools/backend_shard_files.py`, AC-testing.ci-structure.13) | `needs: [changes]` |
| **backend-integration** | Backend integration stage (`-m "integration"`), deterministic service-backed behavior checks | `needs: [changes]` |
| **backend-e2e-tier1** (Shards 1-3) | Backend Tier-1 API E2E stage, split into seeded `least_duration` matrix legs (`ci/backend-tier1-test-durations.json`, AC-testing.ci-structure.14), each uploading `backend-tier1-e2e-N-test-context`: the exact file set is generated, not hand-listed here — see the `backend_tier1_api_e2e` rows in [`common/testing/data/test-execution-matrix.yaml`](data/test-execution-matrix.yaml) (the SSOT view of `common/testing/matrix.py#PATH_RULES`; includes the extraction-corpus journeys, `AC-llm.11`, registered in `common/llm/contract.py`) — with `-m "e2e and not slow and not integration and not perf"`. PR runs stay fail-fast; push/main runs report the full Tier-1 failure set. | `needs: [changes]` |
...
| **tooling-coverage** (Shards 1-2) | Run each shard's slice of root tooling tests (seeded `least_duration` split, `ci/tooling-test-durations.json`) with common/tools coverage and upload each shard's LCOV plus JUnit as `coverage-tooling-N` | `needs: [changes]` |
| **tooling-coverage-merge** | Download all 2 `coverage-tooling-*` shard artifacts, merge their LCOV via `tools/merge_lcov.py` and their JUnit into one `<testsuites>` file, and re-upload the combined `common.lcov`/`tools.lcov`/`backend-tooling.lcov`/`tooling-junit.xml` under the same `coverage-tooling` artifact name downstream jobs already expect | `needs: [changes, tooling-coverage]` |
...
4. **AC Traceability Always Runs**: AC traceability uses `if: always()` so docs-only changes still get static registry/E2E/reconciliation validation even when heavy producers skip; generated registry indexes can be materialized before the audit is built. On heavy CI it waits for lint plus every JUnit producer, downloads the DDD detector artifact and backend/frontend/tooling JUnit, collects a minimized authenticated GitHub issue/ruleset snapshot, and builds one target-SHA package-governance bundle. Detector artifacts cannot contain proof; proof comes from executed declared AC testcases, never `_ac_stubs`, trivial placeholder assertions, pure `pass`, or skipped tests. Inventory claims, `finish.needs`, and the live required-context set must match exactly. Missing/stale evidence or any non-enforced open initiative makes the existing job fail, and `finish` already requires that job. JSON, Markdown, the validated bundle, and the minimized raw GitHub snapshot are uploaded as a CI artifact; no new status context is introduced.
...
8. **Backend and frontend stages are explicit and split**: Backend, frontend, lint, schema, image, and tooling producers still start from change classification in parallel. `ac-traceability` is intentionally a late evidence consumer: on heavy CI it waits for detector/JUnit producers; on lightweight changes `if: always()` preserves its static checks. The finish remains the authoritative aggregate gate.
9. **Coverage Debug Context Is Always Uploaded**: The `tooling-coverage` job uploads common/tools/backend-tooling LCOV plus `tooling-junit.xml` in `coverage-tooling`. Unified coverage consumes the LCOV files; package governance independently consumes the JUnit file. Coverage and execution proof therefore share a producer artifact without sharing policy.
...
18. **Contract checks run in both `lint` and `tooling-coverage` by design**: Each SSOT/contract script the `lint` job runs (`check_manifest`, `check_workflow_contract`, `check_governance_exceptions`, `generate_api_reference --check`, `check_ci_metrics_contract`, `check_ac_index`, `audit_router_contracts`, `lint_doc_consistency`, `generate_epic_status`, etc.) is **also** asserted by a `tests/tooling/` test executed in the `tooling-coverage` job (e.g. `test_check_manifest.py`, `test_workflow_contract.py`), several of which assert that the real repository passes. The two jobs serve different masters and are not redundant: `lint` is the standalone fast-fail lane (isolated `uv --with` overlays rather than the full test environment, surfaces in ~1 min, Key CI Property 1); contract-loading gates such as `check_manifest` and `check_ac_index` explicitly include `pydantic` in that overlay because they instantiate package contracts. `tooling-coverage` executes those contracts as tests so `common/`+`tools/` get line coverage. The consequence is intentional but worth knowing: **a single contract drift fails both `lint` and `tooling-coverage`** — fix the one source, not each job. The `pre-push` tier (see [Local Guardrail Tiers](#local-guardrail-tiers-and-latency-budget)) mirrors the cheapest of these so the drift is usually caught before either CI job runs.
...
| Tier 1 API E2E | `backend-e2e-tier1` job, 3 seeded matrix legs (the `backend_tier1_api_e2e` file set from `common/testing/matrix.py#WORKFLOW_PYTEST_CONTRACTS` with `-m "e2e"`) | Backend contract/HTTP/DB/S3 API behavioral paths with Postgres and MinIO bucket readiness, including the extraction-corpus journeys (`AC-llm.11`, see `common/llm/readme.md#extraction-corpus-e2e`); PR runs use `--maxfail=1`, while push/main Tier-1 E2E runs without `--maxfail=1` so one JUnit artifact reports all failing journeys | Behavioral proof only; AC traceability-backed | Stabilize a deterministic API subset first, then scale by marker or folder |
...
- 4-way parallel test sharding over the `pytest-split` duration seed (consolidated from 8-way alongside pytest -n auto parallelization, saturating runner vCPUs and cutting fixed VM startup overhead while keeping wall-clock runtime low — see AC-testing.ci-structure.8's updated statement). The split is file-level (AC-testing.ci-structure.13): pytest-split's item-level split made every shard import and collect the whole suite under `--cov-branch` first, 37-58 s per shard on a hosted runner (#2050).
- The workflow job name `Backend Tests (Shard ${{ matrix.shard }}/4)` is stable so branch protection and CI metrics retain one consistent status family.
- Each shard: `tools/backend_shard_files.py --splits 4 --group N --splitting-algorithm=least_duration --durations-path ci/backend-test-durations.json` prints the shard's files (every `tests/**/test_*.py` file lands in exactly one shard), then `pytest` runs exactly those files.
...
> |-------------|-------------|------------|----------------|
> | **GitHub CI** | `-n auto` + 8 seeded file-level shards | ~12.5% tests per shard | Medium (ephemeral runners) |
> | **Local CI** | `-n 4` (fixed) | 100% tests | Controlled (shared machine) |
>
> This is intentional design, not inconsistency. CI keeps `-n auto` (2
> workers: `psutil` makes it count physical cores). The hosted runner has 4
> logical CPUs on 2 physical cores, and `-n logical` measured slower in run
> 35181195140: backend collection 58 s -> 98 s, Tier-1 tests 156 s -> 246 s.
