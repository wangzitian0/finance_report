...
8. **Backend and frontend stages are explicit and split**: Backend, frontend, lint, schema, image, and tooling producers still start from change classification in parallel. `ac-traceability` is intentionally a late evidence consumer: on heavy CI it waits for detector/JUnit producers; on lightweight changes `if: always()` preserves its static checks. `finish` remains the authoritative aggregate.
