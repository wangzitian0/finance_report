...
| **backend** (Shards 1-8) | Backend fast-path tests only: `-m "not slow and not e2e and not integration"`, each shard running only the test files its seeded file-level split assigns (`tools/backend_shard_files.py`, AC-testing.ci-structure.13) | `needs: [changes]` |
...
- 8-way parallel test sharding over the `pytest-split` duration seed (raised from 5-way: the committed duration seed had drifted 347 tests stale, and 8-way keeps the slowest shard's wall-clock lower even once the seed is fresh — see AC-testing.ci-structure.8's updated statement). The split is file-level (AC-testing.ci-structure.13): pytest-split's item-level split made every shard import and collect the whole suite under `--cov-branch` first, 37-58 s per shard on a hosted runner (#2050).
...
- Each shard: `tools/backend_shard_files.py --splits 8 --group N --splitting-algorithm=least_duration --durations-path ci/backend-test-durations.json` prints the shard's files (every `tests/**/test_*.py` file lands in exactly one shard), then `pytest` runs exactly those files.
