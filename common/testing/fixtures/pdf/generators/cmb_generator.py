...
from .font_utils import get_safe_font, register_chinese_fonts
...
class CMBGenerator(BasePDFGenerator):
    ...
    def generate(
        ...
        header_font = (
            ...
            else get_safe_font(font_family, self.chinese_font)
        )
        ...
        body_font = (
            self.chinese_font
            if self.chinese_font
            else get_safe_font(self._get_font("body")[0], self.chinese_font)
        )
        ...
        # Period
        period_str = (
            f"{period_start.strftime('%Y-%m-%d')} 至 {period_end.strftime('%Y-%m-%d')}"
        )
        ...
        table_font = (
            self.chinese_font
            if self.chinese_font
            else get_safe_font(self._get_font("table_header")[0], self.chinese_font)
        )
        ...
        table_body_font = (
            self.chinese_font
            if self.chinese_font
            else get_safe_font(self._get_font("table_body")[0], self.chinese_font)
        )
        table_style.add(
            "FONTNAME", (0, 0), (-1, -1), table_body_font
