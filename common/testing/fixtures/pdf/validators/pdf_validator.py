...
class PDFValidator:
        ...
        try:
            with pdfplumber.open(pdf_path) as pdf:
                ...
                if tables:
                    ...
                    if table_config:
                        table_validation = self._validate_table(tables[0], table_config)
    ...
    def validate_real_format_contract(
        ...
        if contract.get("sensitive_data_policy") != "sanitized_format_metadata_only":
            result["success"] = False
            result["errors"].append(
                "real_format_contract must use sanitized_format_metadata_only policy"
            )
        ...
        for key in ("page_size_points", "column_width_points"):
            ...
            if not isinstance(value, (int, float)) or value <= 0:
                result["success"] = False
                result["errors"].append(
                    f"real_format_contract tolerances.{key} must be positive"
                )
        ...
        if not isinstance(date_regex, str) or not date_regex:
            result["success"] = False
            result["errors"].append(
                "real_format_contract source_formats.date_regex is required"
            )
        else:
            try:
                ...
                result["success"] = False
                result["errors"].append(
                    f"real_format_contract date_regex is invalid: {exc}"
                )
        ...
        if not isinstance(min_rows, int) or min_rows <= 0:
            result["success"] = False
            result["errors"].append(
                "real_format_contract transaction_details.min_rows must be positive"
            )
    ...
    def validate_generated_pdf_against_real_format_contract(
        ...
        try:
            with pdfplumber.open(pdf_path) as pdf:
                ...
                if (
                    abs(first_page.width - page_config.get("width", first_page.width))
                    > page_tolerance
                ):
                    result["success"] = False
                    result["errors"].append(
                        "Generated PDF page width is outside real-format tolerance"
                    )
                if (
                    abs(
                        first_page.height - page_config.get("height", first_page.height)
                    )
                    > page_tolerance
                ):
                    result["success"] = False
                    result["errors"].append(
                        "Generated PDF page height is outside real-format tolerance"
                    )
                ...
                if currency not in text:
                    result["success"] = False
                    result["errors"].append(
                        f"Generated PDF does not expose currency marker {currency}"
                    )
                ...
                for phrase in contract.get("key_text", []):
                    if phrase not in text:
                        result["success"] = False
                        result["errors"].append(
                            f"Generated PDF missing key text phrase: {phrase}"
                        )
                ...
                for page in pdf.pages:
                    for table in page.extract_tables() or []:
                        ...
                        for row in table[1:]:
                            normalized = [
                                str(cell or "").strip()
                                for cell in row[:expected_column_count]
                            ]
    ...
    def _header_matches(self, header: list[str], expected_columns: list[str]) -> bool:
        ...
        for actual, expected in zip(header, expected_columns, strict=True):
            ...
            if (
                expected_normalized not in actual_normalized
                and actual_normalized not in expected_normalized
            ):
                return False
        return True
    ...
    def compare_structure(
        ...
        try:
            with (
                pdfplumber.open(real_pdf_path) as real_pdf,
