...
Usage::
    ...
    python tools/build_ac_traceability.py

    # Local guard: fail (exit 1) if the file would change (date-line ignored)
    python tools/build_ac_traceability.py --check

    # Write to a different path (for diffing/preview)
    python tools/build_ac_traceability.py --output /tmp/audit.md
"""
...
def render_document(
    ...
    lines.append("")
    lines.append(
        f"> **Generated**: {today} (mechanically by `tools/build_ac_traceability.py`)"
    )
    lines.append(
    ...
    lines.append(
        ...
        "`tools/build_ac_traceability.py`. CI uploads the generated audit "
    ...
    lines.append("python tools/build_ac_traceability.py")
    lines.append("")
    lines.append("# Local guard (exit 1 if this checked-in archive would change)")
    lines.append("python tools/build_ac_traceability.py --check")
    lines.append("")
    lines.append("# CI artifact preview")
    lines.append(
        "python tools/build_ac_traceability.py --output /tmp/AC-TEST-TRACEABILITY-AUDIT.md"
    )
    lines.append("```")
    lines.append("")
    lines.append(
    ...
    if args.check:
        ...
        if _strip_date(existing) != _strip_date(rendered):
            print(
                ...
                "  Run: python tools/build_ac_traceability.py",
                file=sys.stderr,
            )
            return 1
