...
``--check`` remains available for local refresh validation.

Usage::

    # Regenerate the audit doc in place
    python tools/ssot/build_ac_traceability.py

    # Local guard: fail (exit 1) if the file would change (date-line ignored)
    python tools/ssot/build_ac_traceability.py --check

    # Write to a different path (for diffing/preview)
    python tools/ssot/build_ac_traceability.py --output /tmp/audit.md
"""
...
# ---------------------------------------------------------------------------
# Configuration (mirrors tools/ssot/check_ac_traceability.py)
# ---------------------------------------------------------------------------
...
def render_document(
    ...
    lines.append("")
    lines.append(
        f"> **Generated**: {today} (mechanically by `tools/ssot/build_ac_traceability.py`)"
    )
    lines.append(
    ...
    lines.append(
        ...
        "`tools/ssot/build_ac_traceability.py`. CI uploads the generated audit "
        "as an artifact instead of failing on stale committed archive content. "
        "Update the registries or add an `ACx.y.z` reference inside a test "
        "file, then re-run the builder when intentionally refreshing this "
        "checked-in archive."
    )
    lines.append("")
    lines.append("---")
    ...
    lines.append("python tools/ssot/build_ac_traceability.py")
    lines.append("")
    lines.append("# Local guard (exit 1 if this checked-in archive would change)")
    lines.append("python tools/ssot/build_ac_traceability.py --check")
    lines.append("")
    lines.append("# CI artifact preview")
    lines.append(
        "python tools/ssot/build_ac_traceability.py --output /tmp/AC-TEST-TRACEABILITY-AUDIT.md"
    )
    lines.append("```")
    lines.append("")
    lines.append(
        ...
        "AC reference pattern: `ACx.y.z` (matched by `tools/ssot/check_ac_traceability.py`)."
    )
    lines.append("")
    ...
    if args.check:
        ...
        if _strip_date(existing) != _strip_date(rendered):
            print(
                ...
                "  Run: python tools/ssot/build_ac_traceability.py",
                file=sys.stderr,
            )
            return 1
