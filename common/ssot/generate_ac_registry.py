...
def write_registry(all_acs: dict[str, dict], output_path: str | None = None) -> None:
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        f.write(
            "# DO NOT edit this file manually - run tools/ssot/generate_ac_registry.py\n"
        )
        f.write("# Regenerate: python tools/ssot/generate_ac_registry.py\n")
        f.write(rendered)
    ...
    if args.check:
        if registry_errors:
            ...
            print(
                ...
                f"{details}\n  Run: python tools/ssot/generate_ac_registry.py",
                file=sys.stderr,
            )
            return 1
        if missing_acs:
