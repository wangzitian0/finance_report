...
Two artifacts are emitted and CI-checked for drift, mirroring
``generate_api_reference.py --check``:

* ``docs/ssot/vision-proof-matrix.yaml`` — the parseable matrix.
* ``docs/reference/vision-proof-matrix.md`` — a navigable MkDocs page.

Usage::

    python tools/generate_vision_proof_matrix.py           # write both artifacts
    python tools/generate_vision_proof_matrix.py --check    # fail (1) on drift
"""
