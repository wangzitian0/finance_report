...
Design (mirrors :class:`common.audit.money.Money` / :class:`common.audit.quantity.Quantity`):

- construction rejects ``float``/``bool`` (the standing monetary red line);
- a unit price carries **both** a :class:`~common.audit.money.currency.Currency` and a
  :class:`~common.audit.quantity.unit.Unit`, so applying it to a quantity yields
  :class:`~common.audit.money.money.Money` in the right currency only when the units
  agree;
...
# carve-out in common/audit/money/rounding.py and common/audit/readme.md#base-packages).
UNIT_PRICE_DP: int = 6
