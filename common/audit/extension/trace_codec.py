...
from common.audit.base.trace import TraceRecord, TraceRecordValidationError
    ...
    def decode(raw: str) -> TraceRecord:
        ...
        if unknown:
            raise TraceRecordValidationError(
                f"TraceRecord has unknown fields: {', '.join(unknown)}"
            )
        if missing:
            raise TraceRecordValidationError(
                f"TraceRecord is missing fields: {', '.join(missing)}"
            )
        try:
            ...
            if isinstance(exc, TraceRecordValidationError):
                raise
            raise TraceRecordValidationError(
                f"invalid TraceRecord payload: {exc}"
            ) from exc
