...
from common.audit.money.errors import InvalidCurrencyError

# Active ISO-4217 alphabetic codes. Deliberately a static, dependency-light set
# (no network / no third-party package) so ``common/audit/money`` stays importable
...
class Currency:
    ...
    common.audit.money.errors.InvalidCurrencyError: ...
    """
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(raw, str):
            raise InvalidCurrencyError(
                f"currency code must be a str, got {type(raw).__name__}"
            )
