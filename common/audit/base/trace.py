    ...
    def __post_init__(self) -> None:
        ...
        if not _SHA256_RE.fullmatch(self.assertion_owner_digest):
            raise TraceRecordValidationError(
                "assertion_owner_digest must be a lowercase sha256"
            )
        if self.execution_stage not in _VALID_STAGES:
            raise TraceRecordValidationError(
                f"execution_stage {self.execution_stage!r} is not registered"
            )
        ...
        if self.proof_kind not in valid:
            raise TraceRecordValidationError(
                f"proof_kind {self.proof_kind!r} is invalid for {self.tier}; "
                f"valid kinds: {sorted(valid)}"
            )
        if self.provenance not in {"deterministic", "live_llm", "manual"} and not (
            self.provenance.startswith("golden_fixture@")
            and len(self.provenance) > len("golden_fixture@")
        ):
            raise TraceRecordValidationError(
                "provenance must be deterministic, manual, live_llm, or "
                "golden_fixture@<ref>"
            )
        if (
            self.provenance == "manual"
            and self.execution_stage != "manual.adjudication"
        ):
            raise TraceRecordValidationError(
                "manual provenance requires manual.adjudication"
            )
        if self.tier in {"LLM-LED", "LLM-ONLY"} and self.provenance == "deterministic":
            raise TraceRecordValidationError(
                "LLM-led authority cannot claim deterministic provenance"
            )
        if self.tier == "CODE-ONLY" and self.provenance == "live_llm":
            raise TraceRecordValidationError(
                "CODE-ONLY authority cannot claim live_llm provenance"
            )
        if (
            self.provenance != "manual"
            and self.execution_stage == "manual.adjudication"
            ...
            raise TraceRecordValidationError(
                "manual.adjudication is reserved for provenance='manual'"
            )
...
class TraceDecisionPolicyRegistry:
    ...
    def __post_init__(self) -> None:
        ...
        if len(set(assertions)) != len(assertions):
            raise TraceRecordValidationError(
                "decision policy assertions must be unique"
            )
    ...
    def resolve(self, assertion: VersionedTraceRef) -> TraceDecisionPolicy:
        for policy in self.policies:
            if policy.assertion == assertion:
                return policy
        raise TraceRecordValidationError(
            f"no registered decision policy for {assertion}"
        )
...
class TraceRecord:
        ...
        *,
        ...
        if record_type is TraceRecordType.OBSERVATION:
            ...
            if not parent_ids or len(set(parent_ids)) != len(parent_ids):
                raise TraceRecordValidationError(
                    "DECISION requires non-empty, unique parents"
                )
            if not isinstance(causality, TraceCausality):
                raise TraceRecordValidationError(
                    "DECISION requires a registered causality mode"
                )
        else:
            raise TraceRecordValidationError(
                f"unknown TraceRecord type {record_type!r}"
            )
        _validate_text(execution_id, name="execution_id")
        if not _SHA256_RE.fullmatch(evidence_manifest_digest):
            raise TraceRecordValidationError(
                "evidence_manifest_digest must be a lowercase sha256"
            )
        ...
        *,
        ...
        for parent in parents:
            ...
            if parent.result not in allowed_parent_results:
                raise TraceRecordValidationError(
                    f"unsatisfied decision parent result {parent.result.value!r}"
                )
            if causality is TraceCausality.DIRECT:
                if parent.target != target:
                    raise TraceRecordValidationError(
                        "DIRECT decision parent must pin the same target version"
                    )
                if parent.execution_id != execution_id:
                    raise TraceRecordValidationError(
                        "DIRECT decision parent must share the execution"
                    )
        ...
        has_llm_financial_parent = (
            ...
            and any(
                parent.authority.tier in {"LLM-LED", "LLM-ONLY"} for parent in parents
            )
        )
    ...
    def semantic_payload(self) -> dict[str, Any]:
        return {
            ...
            "score": (
                _decimal_wire(self.score.value) if self.score is not None else None
            ),
    ...
    def restore(cls, payload: dict[str, Any]) -> TraceRecord:
        ...
        if record_type is TraceRecordType.DECISION:
            raise TraceRecordValidationError(
                "DECISION restore requires repository policy replay"
            )
        ...
        record = cls._construct(
            ...
            causality=(
                TraceCausality(payload["causality"])
                if payload["causality"] is not None
                else None
            ),
            ...
            supersedes_id=(
                UUID(payload["supersedes_id"])
                if payload["supersedes_id"] is not None
                else None
            ),
            score=(
                Ratio(Decimal(payload["score"]))
        ...
        if payload["schema_version"] != TRACE_SCHEMA_VERSION:
            raise TraceRecordValidationError(
                f"unsupported TraceRecord schema_version {payload['schema_version']!r}"
            )
...
def current_heads(records: Iterable[TraceRecord]) -> list[TraceRecord]:
    ...
    values = list(records)
    superseded = {
        record.supersedes_id for record in values if record.supersedes_id is not None
    }
    ...
    while pending:
        admitted = [
            record for record in pending if set(record.parent_ids).issubset(valid_ids)
        ]
        if not admitted:
            break
...
def _validate_wire_scalars(payload: dict[str, Any]) -> None:
    ...
    if payload.get("causality") is not None and not isinstance(
        payload["causality"], str
    ):
        raise TraceRecordValidationError(
            "TraceRecord causality must be a string or null"
        )
    ...
    if payload.get("supersedes_id") is not None and not isinstance(
        payload["supersedes_id"], str
    ):
        raise TraceRecordValidationError(
            "TraceRecord supersedes_id must be a string or null"
        )
