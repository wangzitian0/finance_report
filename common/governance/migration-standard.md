...
> and the repo becomes contract-driven. Owned by the `governance`/`meta` package
> (this file is its prose). The only authored horizontal docs that survive are
> `vision.md` and each package's `readme.md`.
...
- [`contract.py`](./contract.py) — the `governance`/`meta` package's own contract.
