...
def _validate_policy_shape(
    ...
    if missing_roots:
        errors.append(
            "source roots are not governed by common/meta/extension/coverage/policy.py: "
            + ", ".join(missing_roots)
        )
...
def _validate_repo_contract_files(repo_root: Path) -> list[str]:
    ...
    if workflow.exists():
        ...
        required_workflow_tokens = (
            ...
            "Upload unified coverage context",
            "unified-coverage-context",
            ...
            "$RUNNER_TEMP/AC-TRACEABILITY-CONTEXT.md",
            ...
            "Backend Tests (Shard ${{ matrix.shard }}/4)",
            "--splits 4",
