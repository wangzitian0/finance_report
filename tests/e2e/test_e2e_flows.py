...
async def test_statement_import_flow(page: Page, repo_root: Path):
    ...
    try:
        ...
        # Check if visible first to avoid error in finally if creation failed
        if await row.count() > 0:
...
async def test_account_deletion_constraint(page: Page):
    ...
    """
    # 1. Create Account
    await authed_page.goto(get_url("/accounts"))
    
    try:
        await authed_page.get_by_role("button", name="Add Account").click()
        await authed_page.fill("input[placeholder='Account Name']", "E2E Constraint Test")
        await authed_page.select_option("select", label="ASSET")
        await authed_page.get_by_role("button", name="Create Account").click()
        await expect(authed_page.get_by_text("E2E Constraint Test")).to_be_visible()
        
        # 2. Create Entry
        await authed_page.goto(get_url("/journal"))
        await authed_page.get_by_role("button", name="New Entry").click()
        await authed_page.get_by_label("Memo *").fill("Constraint Test Entry")
        
        # Use the new account
        selects = authed_page.locator("select")
        # Account 1
        await selects.nth(0).select_option(label="E2E Constraint Test")
        await authed_page.locator("input[type='number']").nth(0).fill("100")
        
        # Account 2 (Any other)
        await selects.nth(2).select_option(index=1)
        await selects.nth(3).select_option(value="CREDIT")
        await authed_page.locator("input[type='number']").nth(1).fill("100")
        
        await authed_page.get_by_role("button", name="Create Entry").click()
        await expect(authed_page.get_by_text("Constraint Test Entry")).to_be_visible()
        
        # 3. Try Delete Account
        await authed_page.goto(get_url("/accounts"))
        row = authed_page.locator("div", has=authed_page.get_by_text("E2E Constraint Test")).last
        ...
        await expect(authed_page.get_by_text("Failed to delete account")).to_be_visible()
        # Verify Account still exists
        await expect(authed_page.get_by_text("E2E Constraint Test")).to_be_visible()
        
        # 4. Delete Entry
        await authed_page.goto(get_url("/journal"))
        entry_row = authed_page.locator("div", has=authed_page.get_by_text("Constraint Test Entry")).first
        authed_page.once("dialog", lambda dialog: dialog.accept())
        await entry_row.get_by_role("button", name="Delete").click()
        await expect(authed_page.get_by_text("Constraint Test Entry")).not_to_be_visible()
        ...
        # Verify Gone
        await expect(authed_page.get_by_text("E2E Constraint Test")).not_to_be_visible()

    finally:
        # Cleanup Entry if exists
        await authed_page.goto(get_url("/journal"))
        entry_row = authed_page.locator("div", has=authed_page.get_by_text("Constraint Test Entry")).first
        if await entry_row.count() > 0:
            authed_page.once("dialog", lambda dialog: dialog.accept())
            await entry_row.get_by_role("button", name="Delete").click()
