...
def test_sanitize_namespace():
    assert test_lifecycle._sanitize_namespace("feat/cleanup-test") == "feat_cleanup_test"
    assert test_lifecycle._sanitize_namespace("PR-123") == "pr_123"
    # Note: implementation removes unknown chars (like spaces) instead of replacing them
...
def test_test_database_ephemeral(mock_get_namespace, mock_get_runtime, mock_run):
    ...
    # call_args[0] is the tuple of positional arguments, which contains the cmd list
