...
@ac_proof(
    proof_id="test_ratio_rejects_float",
    ac_ids=["AC-audit.9.1"],
    ci_tier="pr_ci",
    issue="#1167",
)
def test_AC12_9_1_ratio_rejects_float_and_is_decimal_backed():
    """AC-audit.9.1: Ratio rejects float/bool, is Decimal-backed and immutable."""
...
@ac_proof(
    proof_id="test_ratio_percent_policy",
    ac_ids=["AC-audit.9.1"],
    ci_tier="pr_ci",
    issue="#1167",
)
def test_AC12_9_1_percent_display_is_half_up_two_dp():
    """AC-audit.9.1: percent display is the canonical 2 dp / ROUND_HALF_UP (not HALF_EVEN)."""
    assert PERCENT_DP == 2
...
@ac_proof(
    proof_id="test_ratio_fraction",
    ac_ids=["AC-audit.9.1"],
    ci_tier="pr_ci",
    issue="#1167",
)
def test_AC12_9_1_fraction_and_zero_whole_is_undefined():
    """AC-audit.9.1: fraction(part, whole) builds the ratio; a zero whole raises."""
...
@ac_proof(
    proof_id="test_ratio_arithmetic",
    ac_ids=["AC-audit.9.1"],
    ci_tier="pr_ci",
    issue="#1167",
)
def test_AC12_9_1_dimensionless_arithmetic_and_compare():
    """AC-audit.9.1: ratios add/sub/scale/compare (dimensionless)."""
