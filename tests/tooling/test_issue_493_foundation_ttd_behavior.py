...
from common.ssot import generate_ac_registry as gar  # noqa: E402
...
def test_AC12_19_1_moon_workspace_loads_root_and_app_projects() -> None:
    """AC-platform.19.1: Moon workspace config discovers apps and root tool tasks."""
    ...
    for task in expected_tasks:
        ...
        assert command.startswith("python3 tools/cli.py ")
...
def test_AC14_1_5_smoke_test_succeeds_against_mocked_local_environment(
    ...
    result = subprocess.run(
        [
            "bash",
            str(REPO_ROOT / "tools" / "smoke_test.sh"),
            "http://example.test",
            "prod",
        ],
        check=False,
