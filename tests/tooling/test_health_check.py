...
def test_successful_deployment_prints_generic_guidance_when_observability_backend_unconfigured(
    capsys: pytest.CaptureFixture[str], monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.setattr("common.runtime.health_check.SIGNOZ_URL", "")
    http_get = _router({HEALTH_URL: [(200, '{"status": "healthy"}')]})
    exit_code = check_health(
        HEALTH_URL, "staging", http_get=http_get, sleep=lambda _: None
    )
    ...
    assert out.find("Logs (observability backend):") == -1


def test_successful_deployment_prints_link_when_observability_backend_configured(
    capsys: pytest.CaptureFixture[str], monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.setattr(
        "common.runtime.health_check.SIGNOZ_URL", "https://custom-signoz.example.com"
    )
    http_get = _router({HEALTH_URL: [(200, '{"status": "healthy"}')]})
    exit_code = check_health(
        HEALTH_URL, "staging", http_get=http_get, sleep=lambda _: None
    ...
    _assert_contains(
        out,
        "[SUCCESS] Deployment Successful",
        "Logs (observability backend): https://custom-signoz.example.com",
    )
