"""Tests for tools/analyze_test_ac_coverage.py."""
...
groups:
  ...
  AC6:
    AC6.6:
      - id: AC6.6.6
        epic: 6
        epic_name: tooling-tests
        description: tooling test real
        mandatory: true
  AC7:
    AC7.7:
      - id: AC7.7.7
...
""".strip()
    ...
    def _write_tests(self, repo_root: Path) -> None:
        ...
        tooling_tests = repo_root / "tests" / "tooling"
        stubs = backend / "_ac_stubs"

        backend.mkdir(parents=True, exist_ok=True)
        frontend.mkdir(parents=True, exist_ok=True)
        e2e.mkdir(parents=True, exist_ok=True)
        tooling_tests.mkdir(parents=True, exist_ok=True)
        ...
        (tooling_tests / "test_tooling_contract.py").write_text(
            "# AC6.6.6\n# AC77.7.7\n",
            encoding="utf-8",
        )
        (tooling_tests / "test_generate_ac_registry.py").write_text(
            "# AC99.1.1\n",
            encoding="utf-8",
        )
    ...
    def test_analyze_repo_classifies_real_stub_invalid_and_untested(
        ...
        assert result.source_file_counts["tooling_tests"] == 2
    ...
    def test_render_markdown_contains_required_sections(self, tmp_path: Path) -> None:
        ...
        assert "tooling_tests" in report
