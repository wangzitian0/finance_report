...
be exported by BOTH the backend ratio module (``apps/backend/src/audit/ratio/__init__.py``
``__all__``) and the frontend ratio module (``apps/frontend/src/lib/audit/ratio/index.ts``),
...
def _backend_exports() -> set[str]:
    src = (REPO / "apps/backend/src/audit/ratio/__init__.py").read_text()
...
def _frontend_exports() -> set[str]:
    src = (REPO / "apps/frontend/src/lib/audit/ratio/index.ts").read_text()
    names: set[str] = set()
