...
def test_AC12_37_2_reconciliation_config_sums_lines_as_money():
    ...
    src = _read("apps/backend/src/reconciliation/extension/entry_reads.py")
...
def test_AC12_38_2_balance_core_sums_money():
    """AC-audit.38.2: the balance core builds typed Money in the explicit base context."""
    ...
    assert "Money(line.amount, line.currency or base.code)" in src
    assert "Money.sum(" in src
...
def test_AC12_38_3_annualized_income_reads_line_money():
    ...
    src = _read("apps/backend/src/reporting/extension/annualized_income.py")
    assert "line.money" in src
