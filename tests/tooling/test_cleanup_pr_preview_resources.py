...
def test_AC8_13_38_legacy_cleanup_entrypoints_are_removed() -> None:
    """AC-testing.preview.1: The app performs no Dokploy preview reclaim — on PR close it dispatches a vendor-neutral teardown to infra2 (which owns the 1:1 reclaim); the app k..."""
...
def test_AC8_13_74_maintenance_cleanup_is_ghcr_pruning_only() -> None:
    """AC-testing.preview.6: The app's scheduled maintenance performs no Dokploy preview reconcile and no host hygiene — it only prunes the app's own stale GHCR PR image tags (..."""
