...
SOURCE_CAPABILITIES = ac_index_wrapper.SOURCE_CAPABILITIES
SourceCapability = ac_index_wrapper.SourceCapability
SourceCapabilityStatus = ac_index_wrapper.SourceCapabilityStatus
...
def test_AC_testing_source_capability_proof_2_static_matrix_is_retired(
    ...
    wrapper_source = (ROOT / "tools/check_ac_index.py").read_text(encoding="utf-8")
    assert "run_path(" in wrapper_source
    assert "from src.extraction" not in wrapper_source
