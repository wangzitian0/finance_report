...
def test_AC16_13_12_generate_statement_builds_pdf_rows(monkeypatch, tmp_path):
    ...
    if GENERATE_TEST_PDFS_MODULE in sys.modules:
        del sys.modules[GENERATE_TEST_PDFS_MODULE]
    gtp = importlib.import_module(GENERATE_TEST_PDFS_MODULE)
...
# ---------------------------------------------------------------------------
# Extended tests for tools/generate_test_pdfs.py
# ---------------------------------------------------------------------------
...
def test_main_success_and_font_failure(tmp_path, fake_reportlab, monkeypatch):
    ...
    gtp2 = importlib.import_module(GENERATE_TEST_PDFS_MODULE)
