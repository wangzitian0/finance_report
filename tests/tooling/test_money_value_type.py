...
def test_AC2_20_1_convert_applies_rate_and_changes_currency():
    ...
    result = convert(
        Money(Decimal("100.00"), "USD"), ExchangeRate("USD", "SGD", Decimal("1.35"))
    )
...
def test_AC2_20_1_convert_rounds_half_even_at_boundary():
    ...
    assert convert(
        Money(Decimal("1.00"), "USD"), ExchangeRate("USD", "EUR", Decimal("1.005"))
    ) == Money(Decimal("1.00"), "EUR")
    # 1.015 -> half to even -> 1.02.
    assert convert(
        Money(Decimal("1.00"), "USD"), ExchangeRate("USD", "EUR", Decimal("1.015"))
    ) == Money(Decimal("1.02"), "EUR")
...
def test_AC12_30_3_convert_accepts_typed_exchange_rate():
    """AC-audit.30.3: convert takes an ExchangeRate carrying source/target currency."""
    rate = ExchangeRate("usd", "sgd", Decimal("1.35"))
    assert rate.base == Currency("USD")
    assert rate.quote == Currency("SGD")
    assert convert(Money(Decimal("100.00"), "USD"), rate) == Money(
        Decimal("135.00"), "SGD"
    )
...
def test_AC12_30_3_exchange_rate_source_mismatch_raises():
    """AC-audit.30.3: a rate whose base currency does not match the money source raises."""
    with pytest.raises(CurrencyMismatchError):
        convert(
            Money(Decimal("100.00"), "EUR"), ExchangeRate("USD", "SGD", Decimal("1.35"))
        )
...
def test_AC12_30_3_convert_rejects_naked_decimal_rate():
    """AC-audit.30.3: convert rejects the old naked-Decimal rate boundary."""
