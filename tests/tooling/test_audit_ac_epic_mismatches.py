...
from common.testing import audit_ac_epic_mismatches as aam
...
class TestFixtureClassification:
    def test_tooling_tests_file_is_fixture_file(self, tmp_path):
        tooling_tests = tmp_path / "tests" / "tooling"
        tooling_tests.mkdir(parents=True)
        fixture = tooling_tests / "test_fake_ac_fixture.py"
...
class TestMain:
    ...
    def test_fixture_exclude_label_for_tooling_tests(self, tmp_path, capsys):
        """AC8.13.35: Synthetic tooling-test fixture IDs are not actionable mismatches."""
        self._setup(tmp_path)
        tooling_tests = tmp_path / "tests" / "tooling"
        tooling_tests.mkdir(parents=True)
        (tooling_tests / "test_x.py").write_text("# AC9.9.9\n")
        with mock.patch.object(aam, "ROOT", tmp_path):
            aam.main()
        out = capsys.readouterr().out
        assert "Actionable mismatched refs: **0**" in out
