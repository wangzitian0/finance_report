...
> Note: fixed staging/production deploys run through infra2 `deploy_v2` / `deploy_primitive`. The primitive skips legacy static-token preflight when AppRole creds (`VAULT_ROLE_ID`/`VAULT_SECRET_ID`) are present, and only checks `VAULT_APP_TOKEN` when a non-AppRole compose still carries one. `VAULT_APP_TOKEN` is a legacy fallback only and is unused by current AppRole services.
...
| `VAULT_APP_TOKEN` *(legacy)* | Runtime auth retired; deploy_v2 skips it when AppRole creds are present and checks it only for non-AppRole envs | Unused by AppRole services (all of them today) | Dokploy ENV — legacy fallback only |
...
# 3. Restart service by rerunning the fixed production deploy for the current release tag
cd repo
...
# 3. Deploy
# First register the fixed QA environment in infra2's service registry, then run the
# corresponding deploy_v2 front door. Do not call the retired app-side Dokploy bash path.
...
- name: Deploy preview lifecycle
  ...
  run: |
    # PR preview lifecycle owns ephemeral Dokploy resources
    # Vault path: secret/data/finance_report/pr-47/app
    python tools/pr_preview_lifecycle.py --action deploy --pr "${{ github.event.pull_request.number }}"
```
