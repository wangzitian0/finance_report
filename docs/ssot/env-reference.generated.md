...
| --- | --- | --- | --- | --- |
| `DATABASE_URL` | `postgresql+asyncpg://postgres:postgres@localhost:5432/finance_report` | yes | Database | Async SQLAlchemy database URL. Use postgresql+asyncpg:// for async FastAPI compatibility. |
...
| `DEBUG` | `true` |  | App Settings | Enable debug mode. |
...
| `S3_ENDPOINT` | `http://localhost:9000` | yes | S3 / MinIO Storage | S3 / MinIO endpoint used for storing uploaded statement files. |
| `S3_PRESIGN_EXPIRY_SECONDS` | `900` |  | S3 / MinIO Storage | Presigned URL TTL (seconds) for general S3 access. |
...
| `S3_SECRET_KEY` | `<YOUR_S3_SECRET_KEY>` | yes | S3 / MinIO Storage | S3 / MinIO secret key. |
...
| `SECRET_KEY` | `generate_a_secure_token_for_production_here` | yes | Security | Application secret key. CRITICAL: must be set to a secure random value in production via Vault. |
