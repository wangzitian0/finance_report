...
5. **Coveralls Upload Is Reporting-Only**: Unified, backend, and frontend Coveralls uploads run on both pull requests and `main` pushes when heavy CI is required. CI pass/fail is decided by local gates (`scripts/check_ci_metrics_contract.py`, `scripts/check_coverage_policy.py`, `scripts/calculate_unified_coverage.py`). After those gates pass, CI best-effort publishes reporting-only success for known and discovered Coveralls commit statuses (`coverage/coveralls`, `coverage/coveralls (push)`, `Coveralls - unified`, `Coveralls - backend`, `Coveralls - frontend`, and future `coverage/coveralls ...` or `Coveralls...` contexts) on every SHA GitHub may evaluate for mergeability, including the PR head SHA and synthetic pull-request merge SHA. CI then waits once for late Coveralls writes and publishes the same reporting-only statuses again. Coveralls remains enabled for dashboards and history but does not block merges or post-merge staging. Only the `unified-coverage` job has `statuses: write`.
...
- The asynchronous Coveralls status contexts include `coverage/coveralls`, `coverage/coveralls (push)`, `Coveralls - unified`, `Coveralls - backend`, and `Coveralls - frontend`; CI also discovers future `coverage/coveralls ...` or `Coveralls...` contexts from the commit status API. CI publishes reporting-only success once immediately after local gates pass, waits once for late Coveralls writes, then publishes the same statuses again on every SHA GitHub may evaluate for mergeability. Aggregate contexts such as `coverage/coveralls` and `coverage/coveralls (push)` must not fail CI when absent, delayed, renamed, failed by Coveralls, or hidden behind a transient GitHub status API error after the local deterministic gates pass.
