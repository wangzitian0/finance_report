...
13b. **Gate Inventory During Simplification**: `docs/ssot/ci-gate-inventory.yaml` is a transitional workflow-job inventory for cleanup. Every workflow job has exactly one proof `stage` and one `task_category`; these fields are placement metadata, not AC coverage keys. `finish` remains the only branch-required status context, and resolved duplicate cleanups stay recorded after deletion. `tests/tooling/test_ci_gate_inventory.py` validates that the inventory matches live workflow jobs and `finish.needs`, so a cleanup PR cannot add a replacement entrance while leaving the old one untracked.
