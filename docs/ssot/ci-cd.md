...
- Staging deploy is manual (`workflow_dispatch`) only; a manual dispatch always deploys staging and runs the provider-backed AI/OCR gate against the dispatched ref. The diff-based change classifier no longer scopes the staging deploy by changed paths — it remains the scoping mechanism for CI/PR gates. Normal staging deploys still run staging smoke and non-LLM E2E against the exact dispatched SHA.
...
- Staging deploy is manual (`workflow_dispatch`) only and does not auto-follow main; CI is the development quality gate, not an auto-deploy trigger. The deploy job does not poll or wait for CI. A manual dispatch always performs a real deploy for the chosen `ref` (the change classifier forces staging-required on dispatch); image promotion, the `staging` tag move, and the Dokploy staging change happen only on an explicit dispatch.
...
- Staging checks out the dispatched `ref` input (default `main`) and deploys that exact SHA. Staging is a pre-prod canary: ≈ prod, only slightly ahead.
...
- The AI/OCR gate runs as part of a manual staging dispatch (it inherits the deploy workflow's `workflow_dispatch` trigger) before spending provider quota, and can also be invoked on demand via `staging-ai-ocr-gate.yml`.
...
- The staging lane promotes existing SHA images to the moving `staging` tag on a manual dispatch, avoiding redundant Docker builds when the `:<sha>` image already exists from CI.
