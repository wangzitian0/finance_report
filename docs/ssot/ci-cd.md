...
- PR preview readiness must not rely on Dokploy deployment records as the hard source of truth. After Dokploy returns `Deployment queued`, the lifecycle performs a short, advisory `compose.one` observation to log whether a new deployment record has appeared and whether it is `running` or `done`. If the record is missing, the workflow proceeds to API and frontend readiness instead of failing in the control-plane probe. API and frontend readiness own the final public-route and expected-SHA proof, while Dokploy record logs identify control-plane lag separately.
- PR preview readiness waits for both `/api/health` and `/frontend-version.json?expected=<sha>` to report the expected `github.sha` before browser E2E starts. API readiness logs unbuffered per-attempt route probes for `/api/health`, `/api/ping`, and `/frontend-version.json?expected=<sha>` so failures split into public-route absence, frontend fallback, backend route/health, stale backend SHA, or healthy frontend with missing API route. Each probe line includes `app_readiness_classification` plus `platform_failure_domain`; platform-domain values intentionally align with infra2's Dokploy Route Canary (`dokploy-control-plane`, `dokploy-worker-or-deployment-record`, `docker-runtime`, `traefik-public-route`) so a preview failure can be escalated to the platform canary without reinterpreting raw HTTP errors. App-only states may report `application-runtime` and should stay in this repo. The Traefik API router must have explicit priority above the same-host web router so `/api/*` cannot be shadowed by the frontend route. The frontend image and runtime environment must carry `GIT_COMMIT_SHA`; the frontend readiness request sets a normal `User-Agent` so edge routing does not reject the CI probe before it can read the bundle fingerprint.
