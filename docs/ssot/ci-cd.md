...
| Unit (fast/shard) | `backend` job, 8 shards immediately after change classification | `-m "not slow and not e2e and not integration"` | Feeds unified line coverage (backend component) | Keep as deterministic base and expand shards if needed |
...
- 8-way parallel test sharding via `pytest-split`
- Each shard: `pytest --splits 8 --group N --splitting-algorithm=least_duration`
- Each backend shard stores and uploads `backend-shard-N-durations.json` so future tuning can promote observed durations into a committed duration seed if count-based balancing regresses.
...
**CI Pipeline (Phase 2 backend/fan-in trim, pending observed baseline):**
- Backend fast tests use 8 shards with `least_duration` split assignment and uploaded duration caches.
...
- Record concrete timing from `CI Timing Summary` after this topology lands on `main`; do not replace the observed baseline with estimates.
