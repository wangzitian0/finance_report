...
- The classifier's primary migration contract is structured Env x Stage JSON outputs: `env_stage_required`, `env_stage_reasons`, `env_stage_stages`, `env_stage_files`, and provider-gate JSON outputs. Legacy scalar outputs such as `heavy_required`, `pr_preview_required`, `staging_required`, and `staging_ai_ocr_required` remain as workflow compatibility shims until all GitHub Actions consumers read the structured matrix.
