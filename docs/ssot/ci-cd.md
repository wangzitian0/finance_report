...
6. **Coveralls Upload Is Reporting-Only**: Unified, backend, and frontend Coveralls upload only on non-PR CI runs after heavy local coverage gates pass. PR CI does not upload to Coveralls, so legacy external Coveralls statuses cannot affect PR mergeability. CI pass/fail is decided by local gates (`tools/check_ci_metrics_contract.py`, `tools/check_coverage_policy.py`, `tools/calculate_unified_coverage.py`). Coveralls remains enabled for dashboards and history on push/manual runs, but Coveralls contexts are not required checks and CI does not write synthetic GitHub statuses for them. CI strips branch records before upload so Coveralls percentages track the line-only unified gate. External Coveralls contexts such as `coverage/coveralls`, `coverage/coveralls (push)`, `Coveralls - unified`, `Coveralls - backend`, and `Coveralls - frontend` must not block merges or post-merge staging. The `finish` job writes a coverage gate summary that identifies `Calculate Unified Coverage` plus `finish` as authoritative and calls out that Coveralls may use a different external comparison baseline.
...
- CI keeps Coveralls uploads enabled after local coverage gates pass on non-PR runs; external Coveralls status is informational and does not block `unified-coverage` job success.
...
- Coveralls uploads are reporting-only on non-PR runs and do not block CI pass/fail when local deterministic gates pass.
...
- PR preview cleanup has two lifecycle paths: the PR `closed` event removes the Dokploy stack, the exact PR compose volumes, and GHCR PR images; the scheduled `PR Preview Cleanup` fallback runs only reconciliation for closed or missing PR previews. Scheduled reconciliation must not own generic host hygiene, global Docker volume pruning, `docker system prune --volumes`, build-cache pruning, image pruning, or journal vacuuming.
- `tools/cleanup_pr_preview_resources.py` is a deprecated compatibility entry point and must not perform SSH cleanup or host-wide Docker/journal pruning. Closed-PR leftovers use `tools/pr_preview_lifecycle.py --action reconcile`; host hygiene uses the VPS-local `finance-report-vps-hygiene.timer`.
...
- Generic VPS host hygiene is local to the VPS via `finance-report-vps-hygiene.timer`. It prunes old stopped containers, old build cache, old unused images, old unused networks, oversized Docker json logs, and systemd journal retention without Dokploy or GitHub credentials. This keeps host disk policy independent from PR preview lifecycle ownership.
- PR preview environment validation runs before Dokploy deploy, storage preflight, and E2E tasks. It fails fast when required preview secrets or identifiers are missing or malformed, including `DOKPLOY_API_KEY`, `VPS_SSH_KEY`, `DOKPLOY_API_URL`, `VPS_HOST`, and `PR_NUMBER`; missing preview secrets are configuration failures, not warning-only skips.
- PR preview storage capacity preflight runs after API readiness and before E2E fixture/upload paths. It fails the preview gate when the VPS root filesystem or the PR-specific MinIO `/data` volume has less than 2GiB available, so storage exhaustion is reported before user-flow E2E tests start uploading statements.
