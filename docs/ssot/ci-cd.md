...
- Existing PR preview composes must preserve compose identity, update the allowlisted deploy environment, call Dokploy `compose.stop` to remove stale containers, call `compose.deploy` so existing preview environments pull the commit-scoped images again, and issue a best-effort `compose.start` so the preview route is restored before readiness polling. `compose.stop` failures from Dokploy host command execution are logged with the existing redacted failure contract but do not replace readiness as deploy proof; the workflow continues to `compose.deploy` and lets `/api/health.git_sha` plus `/frontend-version.json` pass or fail the rollout. `compose.start` may outlive the API timeout; the readiness gates, not the start response, are the deploy proof. PR preview routing depends on the compose identity staying stable; stale rollout protection comes from the post-deploy `/api/health.git_sha` and `/frontend-version.json` readiness gates rejecting any older SHA.
