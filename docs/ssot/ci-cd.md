...
- The classifier's primary migration contract is structured Env x Stage JSON outputs over the complete environment axis (`local`, `pr`, `pr-preview`, `staging`, `prd`): `env_stage_required`, `env_stage_reasons`, `env_stage_stages`, `env_stage_files`, and provider-gate JSON outputs. Legacy scalar outputs such as `heavy_required`, `pr_preview_required`, `staging_required`, and `staging_ai_ocr_required` remain as workflow compatibility shims until all GitHub Actions consumers read the structured matrix.
...
- Before deploying, the staging workflow reuses `tools/ci_change_classifier.py` to compare the successful main CI SHA with its parent. Non-runtime documentation, project archive, AC traceability, and tooling-only changes stop after CI/AC proof and write a skip summary; runtime, deploy, E2E, staging workflow, toolchain, and infra-submodule changes still run the full staging deploy and non-LLM E2E. The same classifier emits the staging provider-gate cell and legacy `staging_ai_ocr_required`; only provider, extraction, statement parsing, PDF fixture, AI/OCR workflow, and critical LLM proof changes run the automatic provider-backed AI/OCR gate.
...
- Deploy health covers image build/push, Dokploy rollout, `/api/health`, shell smoke checks, and core non-LLM E2E. Deploy-smoke version checks are excluded from the parallel non-LLM E2E subset because shell smoke/readiness already owns health/version proof before regression tests start.
...
- Automatic provider-backed AI/OCR validation runs as a conditional downstream job in the same serialized post-merge workflow unit when `staging_ai_ocr_required=true`. This keeps staging stable for the SHA under validation: a newer deploy cannot overwrite staging while an older automatic AI/OCR gate is running. When `staging_ai_ocr_required=false`, a successful deploy-health job is enough to close the staging gate for that merge commit.
...
- PR CI does not spend real OCR/LLM quota, but tooling tests must still left-shift deterministic gate risks: every staging AI/OCR gate test must use isolated user fixtures, and contract tests reject shared mutable users before merge.
...
- PR preview non-LLM E2E mirrors the staging non-LLM command shape: `STRICT_E2E_GATES=true`, `--ignore=tests/e2e/test_version_check.py`, marker `(smoke or e2e) and not llm`, and `-n 4` parallelism. The provider-backed `llm` marker remains post-merge only.
...
| Staging | Non-LLM E2E | `STRICT_E2E_GATES=true pytest tests/e2e --ignore=tests/e2e/test_version_check.py -v -m "(smoke or e2e) and not llm" -n 4` | Tests marked `critical` must fail instead of skip |
