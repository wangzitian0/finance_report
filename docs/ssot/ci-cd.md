...
- Existing PR preview composes must be deleted and recreated before Dokploy deploy after allowlisted env verification. New PR preview composes may use `compose.deploy` directly. Existing previews are intentionally replaceable because PR preview state is disposable, and replacement avoids stale Dokploy rollout state leaving the old container serving a stale `/api/health.git_sha`.
