...
10. **Coveralls Is Main-Only Reporting**: Pull requests do not call Coveralls and therefore do not publish external Coveralls status contexts. CI pass/fail is decided by local gates (`tools/check_ci_metrics_contract.py`, `tools/check_coverage_policy.py`, `tools/calculate_unified_coverage.py`) aggregated by `finish`. Main pushes upload only the unified line-only LCOV report to Coveralls for badge and trend reporting after the local coverage gate passes. Backend/frontend per-flag Coveralls uploads are intentionally absent so a single commit has one reporting denominator.
11. **Single CI Metrics Contract**: `tools/check_ci_metrics_contract.py` is the single CI metrics contract. It runs in `lint` and validates that source-root discovery, `common/coverage/policy.py`, workflow gates, and AC traceability semantics stay aligned before coverage jobs finish.
12. **Toolchain Contract**: `tools/check_toolchain_contract.py` runs in lint and fails when Python, Node.js, uv, Docker base images, Compose service images, or frontend engine constraints drift from `toolchain.toml`.
13. **PR Image Build Validation**: PR CI dry-runs staging image builds before merge with the same Dockerfiles, contexts, and build arguments used by `main`. Main push CI is the only path that pushes SHA-tagged images to GHCR.
14. **Coverage Policy Audit**: `tools/check_coverage_policy.py` fails CI if backend, frontend, common, or tools source files drift from their LCOV report.
15. **No-regression gate**: Zero-tolerance; if ANY component is below baseline, CI fails immediately.
16. **Deny-list coverage scope**: Coverage scope is deny-list based within each governed source root. CI recursively expects every eligible source file in backend, frontend, common, and tools LCOV unless `common/coverage/policy.py` explicitly excludes it. New source roots fail the metrics contract until added to the policy and report pipeline.
...
- Every required staging deploy also runs a lightweight AI provider connectivity smoke after deployed health and provider-free E2E pass. This single real chat round trip proves staging Vault/provider credentials, base URL, and primary model routing without running the full PDF OCR replay on every merge. The full OCR/LLM replay remains gated to provider, extraction, statement parsing, PDF fixture, AI/OCR workflow, or critical LLM proof path changes.
- The provider connectivity smoke is resilient to transient provider unavailability so a flaky upstream cannot red `main`. It retries the round trip with exponential backoff, then classifies the last observed outcome: a client/config `4xx` is a hard delivery gate (`config-failure`), while a `5xx`, timeout, or empty response is treated as transient and reported as a non-blocking `degraded` status. A degraded smoke still surfaces in the step summary, the failure-context classifier, and `Post-merge Delivery` as `passed-degraded-provider`, but it does not fail post-merge delivery. Provider configuration regressions therefore still block, transient provider blips do not.
- Staging deploys use an explicit FIFO post-merge train gate instead of workflow-level concurrency. Every successful main CI `workflow_run` is preserved as its own `Deploy Staging` run; before any run mutates staging, `tools/wait_post_merge_train_turn.py` waits for all older active `Deploy Staging` workflow runs to fully complete. The FIFO wait, staging relevance classification, and deploy run in one GitHub Actions job so the workflow does not have to acquire a second runner between ready-to-deploy and staging mutation. This avoids GitHub Actions concurrency's one-pending-run replacement behavior and keeps each merge commit's deploy, smoke, E2E, and required AI/OCR proof as one ordered train unit.
...
- Automatic main post-merge staging failures create or update one persistent GitHub Issue alert titled `[staging-alert] Post-merge staging deploy failing`. Later failures append comments with the target SHA, run URL, staging URL, job results, structured build/deploy failure domain, failed step, failure summary, and current staging SHA; the next successful staging run comments and closes the issue. This is a CI/deploy visibility alert and does not replace SigNoz/Lark runtime alerting.
...
- PR preview deploy preflights required lifecycle inputs and commit-scoped GHCR image tags before making Dokploy mutate the environment. Missing `DOKPLOY_API_KEY`, GitHub integration id, branch/SHA, registry namespace, domain, or backend/frontend image tags are fast-fail setup errors, not Dokploy rollout or application readiness errors. Every deploy attempt writes `ci-context/pr-preview-deploy-context.json` through the lifecycle tool using only redacted, allowlisted fields: phase, PR number, compose name/id, branch, commit SHA, expected SHA, commit-scoped backend/frontend image references, app/API/frontend-version URLs, Dokploy API URL, and environment id. The workflow uploads the whole `ci-context/` directory with `if: always()` so failures before readiness still leave enough evidence to split setup, Dokploy control-plane, rollout, route readiness, and E2E application behavior.
- Existing PR preview composes must preserve the Dokploy compose identity during normal rollout, update the allowlisted deploy environment, and call Dokploy `compose.redeploy` so existing preview environments pull the commit-scoped images again. New preview composes still use `compose.deploy`. The lifecycle must not pre-stop an existing compose on the normal redeploy path: commit-scoped public routes prevent an older container from satisfying newer readiness, and stopping first can disrupt an active Dokploy rollout before Traefik has attached the backend route. The lifecycle must not call Dokploy `compose.start` after `compose.redeploy`: redeploy already queues the rollout, and a second start request can fail independently of the accepted deployment. Existing preview composes with no deployment records are treated as broken empty shells and are deleted/recreated before rollout. If an existing compose accepts `compose.redeploy` but `compose.one` does not expose a new deployment id within the worker-queue observation window, or the new deployment id fails before readiness, the lifecycle deletes and recreates that PR compose once, reapplies the same source and allowlisted environment, then triggers `compose.deploy`. For newly created preview composes, if Dokploy accepts `compose.deploy` but does not expose a deployment record within the short new-record window, CI retries once with `compose.redeploy`; if that still leaves an idle compose with no deployment record, the lifecycle deletes the empty compose shell, recreates it once, reapplies source and allowlisted environment, and triggers one final `compose.deploy`. Once a new deployment id appears, the rollout can use the remaining total rollout window to reach `done` before readiness starts; a late worker record must not be cut off by the deployment-record window itself. Dokploy busy-queue extensions may extend the new-record observation window only inside the total rollout deadline, and the GitHub deploy lifecycle step has its own hard timeout so PR checks cannot remain pending indefinitely. If Dokploy still does not create a deployment record after the accepted deploy request and retry policy, the lifecycle logs `platform_failure_domain=dokploy-control-plane-record-missing`, writes a failed deploy context, and fails before public readiness so CI does not spend the API/frontend readiness window on a compose that never rolled out. If a new deployment id appears and fails, or does not reach `done`, the lifecycle fails before readiness as a platform rollout failure. Each observation logs compose status, deployment count, and new deployment ids without raw response bodies. Deployment or compose error summaries may print only allowlisted deployment diagnostic fields such as message, error, errorMessage, status reason, reason, description, and logPath; these values must be truncated and redacted, and raw compose, deployment, and env payloads must not be printed.
- PR preview public routing keeps a stable canonical host while preserving commit-aware compatibility: lifecycle code still derives `ENV_SUFFIX`, `ENV_DOMAIN_SUFFIX`, `NEXT_PUBLIC_API_URL`, `NEXT_PUBLIC_APP_URL`, database/minio host names, and host port offsets from `pr-<number>-<github.sha[:12]>`, but `app_url` now uses a per-PR stable host `report-pr-<number>.<domain>`. The internal Docker network remains PR-scoped through `NETWORK_SUFFIX=-pr-<number>` so repeated pushes do not allocate one Docker subnet per commit; container names and compose identity stay commit-scoped for rollout safety. Traefik now exposes both stable per-PR routing and commit-compatible legacy aliases so old PR preview links are rerouted to the latest deployed revision instead of showing hard `404` responses. The Dokploy compose remains named `pr-<number>` for PR close cleanup, and readiness/E2E must use the `app_url` output written by `tools/pr_preview_lifecycle.py --action deploy`, not a workflow-local `https://report-pr-<number>.<domain>` string.
- PR preview readiness starts only after Dokploy exposes a new deployment record and that record reaches `done`, or when an existing deployment record leaves the compose in `done` after the new-record observation window. `running` is an intermediate worker state and must not release readiness because Docker containers and Traefik routes can still be absent for the target SHA. Missing deployment records are classified as `dokploy-control-plane-record-missing` and fail before readiness, while unfinished or failed records remain platform rollout failures and should be debugged with infra2's Dokploy Route Canary before app readiness or browser E2E spends time on a stale route. API and frontend readiness remain the final public-route and expected-SHA proof.
- PR preview readiness waits for both `/api/health` and `/frontend-version.json?expected=<sha>` to report the expected `github.sha` before browser E2E starts. API readiness uses a 600-second deadline and logs unbuffered per-attempt route probes for `/api/health`, `/api/ping`, and `/frontend-version.json?expected=<sha>` so failures split into public-route absence, frontend fallback, backend route/health, backend health responses missing `git_sha`/`version`, stale backend SHA, or healthy frontend with missing API route. To support fail-fast readiness and cap the doomed-deploy window, the readiness check exits early with a classified failure domain if consecutive Dokploy worker or deployment record failures reach 8, or if all three readiness endpoints (/api/health, /api/ping, and frontend version endpoint) return HTTP 404 consecutively 15 times. Each probe line includes elapsed seconds, `app_readiness_classification`, and `platform_failure_domain`; platform-domain values intentionally align with infra2's Dokploy Route Canary (`dokploy-control-plane`, `dokploy-worker-or-deployment-record`, `docker-runtime`, `traefik-public-route`) so a preview failure can be escalated to the platform canary without reinterpreting raw HTTP errors. Readiness probes must use bounded `curl` calls with connect/max-time limits, subprocess timeouts, and an outer workflow step timeout so a healthy public route cannot leave CI stuck in a Python network call. The probes must split response bodies from curl status metadata with an explicit marker rather than a generic trailing newline, send `Accept: application/json`, preserve the full API response body for JSON parsing, and log only the API response content type, body byte count, and a short escaped body prefix when a 200 response is missing `git_sha`/`version`; this makes runner-side parser failures, frontend/edge fallback bodies, and backend health payload gaps distinguishable from the same `route_probe` line without corrupting valid health JSON. Repeated route/backend misses emit a notice after the frontend serves the target SHA, but they must not cut short the full readiness window because backend cold start can lag behind frontend route attachment. App-only states may report `application-runtime` and should stay in this repo. The Traefik API router must have explicit priority above the same-host web router so `/api/*` cannot be shadowed by the frontend route. The frontend image and runtime environment must carry `GIT_COMMIT_SHA`; the frontend readiness request sets a normal `User-Agent` so edge routing does not reject the CI probe before it can read the bundle fingerprint.
...
- PR preview cleanup has two lifecycle paths: the PR `closed` event removes the Dokploy stack and GHCR PR images; the scheduled `PR Preview Cleanup` fallback runs Dokploy compose reconciliation for closed or missing PR previews and prunes closed-PR backend/frontend GHCR tags matching `pr-<number>-<sha>` after the 14-day retention window while preserving tags for open PRs. GitHub workflows must not SSH to the VPS for PR cleanup. Scheduled reconciliation must not own generic host hygiene, global Docker volume pruning, `docker system prune --volumes`, build-cache pruning, image pruning, or journal vacuuming.
