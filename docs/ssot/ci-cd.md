...
- Existing PR preview composes must preserve compose identity during normal rollout, update the allowlisted deploy environment, call Dokploy `compose.stop` to remove stale containers, call Dokploy `compose.redeploy` so existing preview environments pull the commit-scoped images again, and issue a best-effort `compose.start` so the preview route is restored before readiness polling. New preview composes still use `compose.deploy`. `compose.stop` failures from Dokploy host command execution are logged with the existing redacted failure contract and trigger a deterministic fallback that deletes and recreates the same `pr-<number>` compose before deploying; this is the only path allowed to replace an existing preview compose identity. `compose.start` may outlive the API timeout; the readiness gates, not the start response, are the deploy proof. Stale rollout protection comes from the post-deploy `/api/health.git_sha` and `/frontend-version.json` readiness gates rejecting any older SHA.
