...
- Manual `workflow_dispatch` with `dry_run=true` resolves the target release `version_ref`, verifies the tag commit has successful `main` CI, verifies `release-images.yml` published the release images, verifies a successful staging deploy for that same `version_ref`, runs release lint, checks migration risk, and fetches release image digests without mutating production.
