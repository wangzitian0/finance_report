...
- The asynchronous Coveralls status contexts include `coverage/coveralls`, `coverage/coveralls (push)`, `Coveralls - unified`, `Coveralls - backend`, and `Coveralls - frontend`; CI also discovers future `coverage/coveralls ...` or `Coveralls...` contexts from the commit status API. CI waits briefly for stable contexts only, publishes reporting-only success statuses for known and discovered contexts on every SHA GitHub may evaluate for mergeability, performs a settle-time recheck, republishes any late failures, and fails closed if the final latest Coveralls statuses are not the local reporting-only success values. Optional contexts such as `coverage/coveralls (push)` must not extend CI duration when Coveralls does not emit them.
