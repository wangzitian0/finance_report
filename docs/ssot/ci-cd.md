...
- Existing PR preview composes must call Dokploy `compose.redeploy` after allowlisted env verification. New PR preview composes may use `compose.deploy`; existing previews need the redeploy endpoint so a successful deploy trigger cannot leave the old container serving a stale `/api/health.git_sha`.
