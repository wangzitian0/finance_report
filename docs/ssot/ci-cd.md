...
20. **`container-images` is right-moved to build-context changes**: The classifier (`common/ci/change_classifier.py`) emits `image_build_required`, true only when a Docker build input changed — Dockerfiles, dependency manifests/locks (`uv.lock`, `package-lock.json`, `pyproject.toml`, `package.json`), build config (`next.config.mjs`, `tsconfig.json`, `postcss`/`tailwind` config), or the backend entrypoint/build scripts (`apps/backend/scripts/`). A pure app-source change is `COPY`'d into the image and is already proven by `frontend-build` (`tsc --noEmit` + `next build`) and the backend test jobs, so it does **not** trigger a fresh image build. `container-images` gates on `pr_required && image_build_required`; `finish` treats a skipped `container-images` as a pass (not a gap). An unknown diff fails closed and rebuilds. This trades a small amount of per-PR image re-validation for not rebuilding both images on every source-only PR.
