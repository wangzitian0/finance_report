...
**CI Pipeline (Phase 1 frontend split, pending observed baseline):**
- Frontend PR CI is split into `frontend-build`, `frontend-vitest`, `frontend-playwright`, and `frontend-telemetry-e2e`, all starting after `changes`.
- `unified-coverage` and `ac-behavioral-ratchet` wait only for `frontend-vitest`; `finish` remains the merge authority for all four frontend gates.
- Playwright and telemetry jobs run their own `npm run build` before `npm run start` because the CI Playwright configs start an already-built app. This duplicates build compute, but removes the old serial wait behind Vitest and should reduce the heavy CI critical path when runners are available.
