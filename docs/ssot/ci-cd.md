...
- PR preview deploy builds and pushes commit-scoped PR backend and frontend images before invoking Dokploy. The preview compose uses `IMAGE_TAG=pr-<number>-<github.sha>`, so the deploy job must make `finance_report-backend:pr-<number>-<github.sha>` and `finance_report-frontend:pr-<number>-<github.sha>` available in GHCR before `tools/pr_preview_lifecycle.py --action deploy` can trigger the rollout. PR close cleanup deletes GHCR tags with prefix `pr-<number>-` for backend/frontend. The root compose must also pass `GIT_COMMIT_SHA` into the frontend build so `apps/frontend/Dockerfile` can bake `/frontend-version.json`. Before E2E starts, the workflow waits for both `/api/health` and `/frontend-version.json` to report the expected `github.sha`; the frontend readiness request sets a normal `User-Agent` so edge routing does not reject the CI probe before it can read the bundle fingerprint. This prevents backend-only readiness from running browser tests against a stale frontend bundle.
