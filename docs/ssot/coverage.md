...
- **External reporting**: Coveralls remains enabled for historical visibility, but local deterministic gates decide whether CI fails. `scripts/mark_coveralls_reporting_status.py` waits for Coveralls commit statuses, publishes reporting-only success statuses on every PR SHA GitHub may evaluate for mergeability, and rechecks after a settle period so late Coveralls aggregate writes cannot override the local gate result.
