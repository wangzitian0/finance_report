...
> **Purpose**: Local development, testing, CI, and deployment workflows.
...
| Rule | Description |
|------|-------------|
| **Coverage Gate** | Backend tests must maintain >= 95% line coverage |
| **NEXT_PUBLIC_ Baking** | All frontend env vars must be in Dockerfile ARG |
| **Env Var Consistency** | `secrets.ctmpl` ↔ `config.py` ↔ `.env.example` must align |
| **Submodule Sync** | Production config changes require `infra2` PR |
| **Container Isolation** | PR containers use unique names (`-pr-XX` suffix) |
...
# One-button CI check (recommended before PR)
moon run :ci
```
...
moon run backend:test
# Coverage must be >= 95% or tests fail
```
