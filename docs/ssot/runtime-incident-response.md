...
| Logs or alerts missing while app serves traffic | `/health.observability`, service name, alert rule, observability backend health | `observability` | App observability contract + infra alerting |
...
| `observability` | App serves but logs, traces, alert rule, or observability-backend/Lark delivery is broken | `/health.observability`, observability backend health/version, alert rule metadata | the observability backend or Lark platform health is degraded |
...
- `docs/ssot/observability.md` owns structured logging, OTEL/the observability backend config, and
