...
| **16** | Credit Card Repayment & Refund Clearing<br>*(信用卡还款勾稽)* | `/reconciliation` | `Workbench` | `POST /reconciliation/matches/{match_id}/accept` | `Bank Outflow (Dr CreditCardLiability) == Credit Card Payment Inflow`. | [`apps/backend/tests/reconciliation/test_credit_card_reconciliation_flow.py`](../../apps/backend/tests/reconciliation/test_credit_card_reconciliation_flow.py) |
...
| **26** | Cash Flow Statement Multi-Activity Invariant Verification<br>*(现金流量表健康度与自洽检验)* | `/reports/cash-flow` | `CashFlowPage` | `GET /reports/cash-flow` | `Net Cash Change == Operating CF + Investing CF + Financing CF`. | [`apps/backend/tests/reporting/test_cash_flow_invariants.py`](../../apps/backend/tests/reporting/test_cash_flow_invariants.py) |
