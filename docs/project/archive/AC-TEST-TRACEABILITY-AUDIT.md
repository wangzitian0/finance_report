...
> **Purpose**: Complete mapping of every Acceptance Criterion (`ACx.y.z`) declared in `docs/ac_registry.yaml` + `docs/infra_registry.yaml` to the test file(s) that reference it. This is not behavioral coverage; use it together with line coverage and product-level E2E evidence.
...
|--------|-------|------------|
| **Total EPICs** | 18 | 100% |
| **Total ACs (registries)** | 958 | 100% |
| **Mandatory ACs** | 798 | 83.3% |
| **Deprecated ACs** | 3 | 0.3% |
| **Mandatory ACs with test reference** | 795 | 99.6% |
...
| [EPIC-008](#epic-008-testing-strategy) | testing-strategy | 87 | 0 | 81 | 78 | 96.3% |
...
- **Total ACs**: 87
- **Mandatory ACs**: 81
- **Mandatory ACs with test reference**: 78 (96.3%)
...
| AC8.13.21 | yes | Provider-backed post-merge AI/OCR gate waits for the same SHA's CI success before running. | `scripts/tests/test_post_merge_e2e_gates.py`<br>`scripts/tests/test_wait_for_github_ci.py` | ✅ |
| AC8.13.22 | yes | Staging deploy waits for the same SHA's CI success before building or deploying. | `scripts/tests/test_post_merge_e2e_gates.py`<br>`scripts/tests/test_wait_for_github_ci.py` | ✅ |
| AC8.13.23 | yes | Automatic staging deploy health and AI/OCR validation run in one serialized post-merge workflow unit. | `scripts/tests/test_post_merge_e2e_gates.py` | ✅ |
...
| AC8.13.27 | yes | Deterministic upload-to-dashboard gate runs as a critical fresh-user staging E2E. | `scripts/tests/test_post_merge_e2e_gates.py` | ✅ |
| AC8.13.28 | yes | Stage 1 review auto-posts journal entries from the deterministic fixture. | _none_ | ❌ missing |
| AC8.13.29 | yes | Reconciliation rerun is idempotent and Stage 2 run review reaches a cleared completion state. | _none_ | ❌ missing |
| AC8.13.30 | yes | Processing Account summary and pending page stay visible and correct for the cleared run. | _none_ | ❌ missing |
| AC8.13.31 | yes | Dashboard, balance sheet, income statement, and cash-flow totals exactly match the deterministic upload fixture. | `scripts/tests/test_post_merge_e2e_gates.py` | ✅ |
