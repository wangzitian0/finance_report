...
| AC12.34.5 | Every posting path is balance-guaranteed by `Entry`: opening-balance and fx-revaluation construct an `Entry` before persisting (the latter previously wrote raw `JournalLine`s with no balance validation at all), closing the double-entry-integrity gap on the last two paths | `test_AC12_34_5_remaining_posting_paths_guard_balance_with_entry` | `tests/tooling/test_ledger_module.py` | P1 |
