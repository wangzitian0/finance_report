...
| AC12.34.4 | Adoption: investment buy/sell/dividend build typed `Entry` and post via `ledger.post_entry` instead of hand-rolling balanced `lines_data` dicts (the legacy `create_journal_entry`/`_post_and_load` path is gone from the service) {tier:PC} | `test_AC12_34_4_investment_postings_use_ledger_post` | `tests/tooling/test_ledger_module.py` | P1 |
| AC12.34.5 | Every posting path is balance-guaranteed by `Entry`: opening-balance and fx-revaluation construct an `Entry` before persisting (the latter previously wrote raw `JournalLine`s with no balance validation at all), closing the double-entry-integrity gap on the last two paths {tier:PC} | `test_AC12_34_5_remaining_posting_paths_guard_balance_with_entry` | `tests/tooling/test_ledger_module.py` | P1 |
