...
| AC12.31.7 | Backend Quantity migration cleanup centralizes service quantization/zero adapters in `src.quantity` and removes service-local `_quantity*` facades from assets, portfolio, reporting, investment accounting, and market data | `test_AC12_31_7_backend_quantity_adapters_are_centralized`, `test_AC12_31_7_backend_quantity_storage_helpers_quantize_and_guard_inputs` | `tests/tooling/test_base_package_migration_cleanup.py` | P1 |
