...
> **Status**: ✅ Complete — shipped across EPIC A/B and cut over to the `llm` package (#1426); the frozen contract (`src/llm/base`, formerly `src/llm/base`) and
...
| Slice | PR | Owns |
|-------|----|------|
| **common** | PR1 | `src/llm/base` (was `src/llm/base`): value types, `ConfigSource`/`LLMClient`/`CatalogProvider` protocols, `SecretCipher`+`FernetCipher` (single-pass key rotation, AC-llm.1 group), [`common/llm/readme.md`](https://github.com/wangzitian0/finance_report/blob/main/common/llm/readme.md) (the LLM SSOT, internalized into the `llm` package). The frozen contract A and B build against. |
