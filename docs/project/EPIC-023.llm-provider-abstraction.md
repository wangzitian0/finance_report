...
| AC23.4.5 | The scene-keyed `LitellmClient` is on the live path: a user's per-scene binding selects the model for that scene, falling back to the deployment default and then the env model | `apps/backend/tests/integration/test_llm_api.py` | P1 |
