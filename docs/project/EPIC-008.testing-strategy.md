...
| AC8.13.131 | The AC traceability report labels its mandatory-coverage number as L1 reference hygiene (reference exists), never as behavioral coverage, and its pass message names the separate L2 critical-proof-matrix and L3 behavioral-score-ratchet gates as the behavioral authorities — so a passing traceability gate cannot be read as misleading behavioral assurance | `test_AC8_13_131_report_labels_l1_hygiene_not_behavioral`, `test_AC8_13_131_pass_message_points_to_behavioral_gates` | `tests/tooling/test_check_ac_traceability.py` | P0 |
| AC8.13.132 | A content-level secret scan (gitleaks) runs in both the pre-commit hooks and the CI `lint` job (local==CI parity), blocking credential material by content rather than by filename so `.gitignore` is not the only line of defense | `test_AC8_13_132_gitleaks_runs_in_precommit_and_ci` | `tests/tooling/test_secret_scan_gate.py` | P0 |
