...
| AC8.13.89 | PR preview follows the matching successful PR `CI` `workflow_run`, runs the runner-local full-stack smoke/E2E preview with the PR head SHA, and does not build, push, preflight, or delete PR preview images | `test_AC8_13_89_pr_preview_follows_ci_without_pr_image_builds` | `tests/tooling/test_post_merge_e2e_gates.py` | P0 |
...
| AC8.13.101 | Current PR preview E2E consumes the runner-local `http://localhost:8080` URL and reports that no persistent Dokploy URL exists; legacy lifecycle helpers still preserve stable/commit preview URL derivation for compatibility cleanup tests | `test_AC8_13_101_preview_app_url_prefers_stable_alias`, `test_AC8_13_101_pr_test_workflow_uses_runner_preview_url` | `tests/tooling/test_pr_preview_lifecycle.py` | P0 |
