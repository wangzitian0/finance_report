...
| AC8.13.89 | PR preview deploy builds and pushes commit-scoped PR backend/frontend images, creates and updates Dokploy with the GitHub-source no-build preview compose file, then gates E2E on backend and frontend version readiness before browser tests run | `test_AC8_13_89_pr_preview_builds_pr_tagged_images_before_deploy`, `test_AC8_13_89_create_compose_initializes_github_no_build_preview_source`, `test_AC8_13_89_preview_source_uses_github_no_build_preview_compose`, `test_AC8_13_89_pr_preview_compose_cannot_build_app_images` | `tests/tooling/test_post_merge_e2e_gates.py`, `tests/tooling/test_pr_preview_lifecycle.py` | P0 |
...
| AC8.13.98 | Existing PR preview composes are deleted and recreated on each deploy before applying allowlisted deploy env and re-running Dokploy `compose.deploy`, then deployment acceptance must materialize as a Dokploy deployment record before HTTP readiness starts, so readiness can reject stale SHA rollouts without depending on `compose.stop` or `compose.start`; the preview compose does not profile-gate services because Dokploy deploy/start paths must select services without relying on `COMPOSE_PROFILES` propagation | `test_AC8_13_98_existing_deployed_preview_compose_is_recreated_without_start_stop`, `test_AC8_13_98_existing_preview_without_deployments_is_recreated`, `test_AC8_13_98_deploy_action_writes_output_before_deployment_record_wait`, `test_AC8_13_98_wait_for_deployment_record_fails_fast_without_raw_response`, `test_AC8_13_89_pr_preview_compose_cannot_build_app_images` | `tests/tooling/test_pr_preview_lifecycle.py` | P0 |
