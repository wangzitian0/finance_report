...
| AC8.13.46 | PR preview non-LLM E2E uses the same strict, parallel gate shape as staging non-LLM E2E, while both exclude the deploy-smoke version test from the parallel regression subset | `test_AC8_13_46_pr_preview_non_llm_gate_matches_staging_strict_parallelism` | `tests/tooling/test_post_merge_e2e_gates.py` | P1 |
...
| AC8.13.107 | PR preview deploy fails before Dokploy when required inputs or commit-scoped GHCR images are missing, and every deploy attempt uploads a redacted context artifact that preserves phase, compose, route, image, and expected-SHA evidence for failure triage | `test_AC8_13_107_deploy_action_fails_fast_on_missing_required_inputs`, `test_AC8_13_107_preview_deploy_context_is_written_without_secrets`, `test_AC8_13_107_pr_preview_workflow_fast_fails_missing_images_and_uploads_context` | `tests/tooling/test_pr_preview_lifecycle.py` | P0 |
