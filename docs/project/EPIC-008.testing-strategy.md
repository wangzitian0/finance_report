...
| AC8.13.100 | PR preview API readiness logs route diagnostics that split frontend route, API route, backend health, stale SHA, and frontend fallback failures, with unbuffered per-attempt output, infra2 Dokploy Route Canary failure-domain hints, and early failure after repeated classified route misses | `test_AC8_13_100_pr_preview_api_readiness_logs_route_diagnostics` | `tests/tooling/test_pr_preview_lifecycle.py` | P0 |
...
| AC8.13.102 | PR preview Dokploy deploys consume CI-built GHCR images with `--pull always --no-build`, log Dokploy deployment record lag, and use commit-scoped readiness as the final public-route and expected-SHA proof | `test_AC8_13_102_dokploy_deploy_waits_for_worker_running_status`, `test_AC8_13_102_dokploy_rollout_timeout_fails_before_readiness`, `test_AC8_13_102_dokploy_rollout_error_fails_before_readiness`, `test_AC8_13_102_existing_preview_rollout_tracks_new_deployment_ids`, `test_AC8_13_102_deploy_record_lag_continues_to_commit_scoped_readiness` | `tests/tooling/test_pr_preview_lifecycle.py` | P0 |
