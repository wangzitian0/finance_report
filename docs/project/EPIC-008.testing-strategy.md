...
| AC8.13.22 | Staging deploys an explicitly supplied published release tag (`inputs.tag`) on `workflow_dispatch`; it does not build or promote images inside the deploy workflow | `test_AC8_13_22_staging_deploys_manually_dispatched_release_tag` | `tests/tooling/test_post_merge_e2e_gates.py` | P0 |
...
| AC8.13.93 | Staging is mutated only by an explicit manual `workflow_dispatch` with a required release `tag` input; no auto path can promote images or change Dokploy, and structured deploy failure context is preserved | `test_AC8_13_93_staging_promotion_requires_manual_dispatch` | `tests/tooling/test_post_merge_e2e_gates.py` | P0 |
...
| AC8.13.142 | CI simplification uses a MECE gate inventory: every workflow job has exactly one category (`aggregate`, `classify`, `static_contract`, `runtime_test`, `evidence_fan_in`, `audit_artifact`, or `deploy_ops`), the inventory matches live workflow jobs and `finish.needs`, and resolved duplicate cleanups stay recorded after deletion so cleanup PRs do not leave both old and new entrances behind | `test_AC8_13_142_ci_gate_inventory_uses_one_mece_category_per_job`, `test_AC8_13_142_finish_inventory_matches_ci_fan_in`, `test_AC8_13_142_duplicate_cleanup_is_explicit_not_implicit` | `tests/tooling/test_ci_gate_inventory.py` | P1 |
...
| AC8.13.148 | Backend fast-test CI shards rebalance the current critical path with an 8-way `pytest-split` matrix, least-duration assignment, and uploaded per-shard duration caches for the next tuning pass | `test_AC8_13_148_backend_shards_use_balanced_8_way_split` | `tests/tooling/test_post_merge_e2e_gates.py` | P1 |
