...
| AC8.13.22 | Staging deploy builds and deploys the manually dispatched `ref` (checkout `inputs.ref \|\| github.sha`) on `workflow_dispatch` | `test_AC8_13_22_staging_deploy_builds_from_dispatched_ref` | `tests/tooling/test_post_merge_e2e_gates.py` | P0 |
...
| AC8.13.36 | Main CI builds SHA-tagged staging images and staging reuses them on a manual dispatch, promoting the `:<sha>` image to the moving `staging` tag and only building the missing image | `test_AC8_13_36_post_merge_reuses_sha_tagged_staging_images` | `tests/tooling/test_post_merge_e2e_gates.py` | P0 |
...
| AC8.13.51 | Staging deploy is manual (`workflow_dispatch`) only with a `ref` input; it does not auto-follow main CI and does not poll for CI in-job | `test_AC8_13_51_staging_deploy_is_manual_dispatch_only` | `tests/tooling/test_post_merge_e2e_gates.py` | P0 |
...
| AC8.13.93 | Staging is mutated only by an explicit manual `workflow_dispatch` (with a `ref` input); no auto path can promote images or change Dokploy, and structured deploy failure context is preserved | `test_AC8_13_93_staging_promotion_requires_manual_dispatch` | `tests/tooling/test_post_merge_e2e_gates.py` | P0 |
...
| AC8.13.142 | CI simplification starts with a MECE gate inventory: every workflow job has exactly one category (`aggregate`, `classify`, `static_contract`, `runtime_test`, `evidence_fan_in`, `audit_artifact`, or `deploy_ops`), the inventory matches live workflow jobs and `finish.needs`, and duplicate candidates are recorded before any deletion so cleanup PRs do not leave both old and new entrances behind | `test_AC8_13_142_ci_gate_inventory_uses_one_mece_category_per_job`, `test_AC8_13_142_finish_inventory_matches_ci_fan_in`, `test_AC8_13_142_duplicate_candidates_are_explicit_not_implicit` | `tests/tooling/test_ci_gate_inventory.py` | P1 |
