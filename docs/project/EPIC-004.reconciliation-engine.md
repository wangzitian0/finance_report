...
| ID | Test Case | Test Function | File | Priority |
|----|-----------|---------------|------|----------|
| AC4.8.1 | Archive baseline benchmark residual is explicitly owned by EPIC-004 until synthetic accuracy and performance proof exists | `test_AC4_8_1_reconciliation_benchmark_residual_is_epic_owned` | `scripts/tests/test_archive_residual_epic_ownership.py` | P1 |

**Traceability Result**:
- Current AC counts and test-reference status are generated by
  `python scripts/analyze_test_ac_coverage.py --stdout` and
