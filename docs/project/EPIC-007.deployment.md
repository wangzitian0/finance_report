...
| AC7.12.4 | App publishes the artifact: `ci.yml` pushes `:<sha>` for `main` and release-branch (`release/**`) commits, and `workflow_dispatch` can publish an arbitrary sha on demand (dropping per-PR auto-preview is P1a-2) | `test_AC7_12_4_release_branches_trigger_the_publish_workflow`, `test_AC7_12_4_image_push_publishes_for_main_release_and_dispatch` | `tests/tooling/test_publish_only_contract.py` | P1 |
