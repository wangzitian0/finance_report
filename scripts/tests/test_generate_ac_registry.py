...
class TestMain:
    ...
    def test_append_registry_entries_expands_empty_acs_list(self, tmp_path):
    ...
    def test_main_check_fails_when_registry_total_is_stale(
        self, tmp_path, monkeypatch
    ):
        """AC8.13.17: Check mode catches stale registry total fields."""
        epic_dir = self._setup_epic_dir(tmp_path)
        (epic_dir / "EPIC-008.testing-strategy.md").write_text(
            "| AC8.13.17 | Append-only generator behavior |\n"
        )
    ...
    def test_main_repairs_stale_registry_total(self, tmp_path, monkeypatch):
        """AC8.13.17: Normal generation repairs stale totals without entry rewrites."""
        epic_dir = self._setup_epic_dir(tmp_path)
        (epic_dir / "EPIC-008.testing-strategy.md").write_text(
            "| AC8.13.17 | Append-only generator behavior |\n"
        )
