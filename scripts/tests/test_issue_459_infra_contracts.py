...
PDF_FIXTURES = REPO_ROOT / "scripts" / "pdf_fixtures"
...
def test_epic_012_and_014_tooling_contracts() -> None:
    ...
    assert (REPO_ROOT / "scripts" / "validate_schemas.py").exists()
    assert (REPO_ROOT / "scripts" / "check_env_keys.py").exists()
    assert (REPO_ROOT / "scripts" / "smoke_test.sh").exists()
    assert (REPO_ROOT / "scripts" / "generate_ac_registry.py").exists()
