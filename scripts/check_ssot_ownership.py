...
SCAN_SKIP_DIRS: set[str] = {
    ...
    ".pytest_cache",
    ".git",
    "archive",  # defensive skip; repository archive snapshots must be absent
}
        ...
        if path.exists():
            violations.append(
                Violation(
                    ...
                    message=(
                        f"{path.relative_to(REPO_ROOT)} must not exist — "
                        "it was merged, renamed, or retired per SSOT "
...
def _collect_scan_files() -> list[Path]:
    """Return all non-excluded markdown/Python files."""
    results: list[Path] = []
