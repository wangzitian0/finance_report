...
def format_timing_summary(run: dict[str, Any], *, title: str) -> str:
    ...
    first_started = min((job.started_at for job in started_jobs if job.started_at), default=None)
    last_completed = max((job.completed_at for job in completed_jobs if job.completed_at), default=updated_at)
    ...
    if first_started and last_completed:
        execution_seconds = max(0, int((last_completed - first_started).total_seconds()))
    ...
    longest = max((job for job in completed_jobs if job.duration_seconds is not None), key=lambda j: j.duration_seconds or 0, default=None)
    ...
    if longest is not None:
        lines.append(f"- Longest completed job: `{longest.name}` at `{format_duration(longest.duration_seconds)}`")
    ...
    for job in sorted_jobs:
        started = job.started_at.isoformat().replace("+00:00", "Z") if job.started_at else "n/a"
        completed = job.completed_at.isoformat().replace("+00:00", "Z") if job.completed_at else "n/a"
